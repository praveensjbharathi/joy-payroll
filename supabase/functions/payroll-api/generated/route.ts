...
import { calculateRevenue, type RevenueAssumptions } from "./revenue-calculator.ts";
import { calculateRevenueProjection, calculateRevenueScenario, defaultRevenueMonth, indiaToday, monthDates, dateOnly, shiftDate, validateRevenueMonth, type RevenueMonthPlan, type RevenueScenarioInput } from "./revenue-projection.ts";
...
import { mutationCollections } from "./mutation-updates.ts";
...
export async function POST(request: Request, identity: AuthenticatedUser | null) {
  try {
    ...
    if (/UNIQUE constraint failed: vendors\.code|vendors_code_(?:unique|key)/i.test(combined))
      return Response.json(
        { error: "This client code already exists" },
        { status: 409 },
      );
    if (
      /UNIQUE constraint failed: shift_definitions\.vendor_id, shift_definitions\.name|shift_client_name_unique/i.test(
        combined,
      )
    )
      return Response.json(
    ...
    if (/UNIQUE constraint failed: employees\.employee_code|employees_employee_code_(?:unique|key)/i.test(combined))
      return Response.json(
