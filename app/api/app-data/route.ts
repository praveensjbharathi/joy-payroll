...
export async function POST(request: Request) {
  try {
    ...
    if (/UNIQUE constraint failed: vendors\.code/i.test(combined)) return Response.json({ error: "This client code already exists" }, { status: 409 });
    if (/UNIQUE constraint failed: shift_definitions\.vendor_id, shift_definitions\.name/i.test(combined)) return Response.json({ error: "This shift name already exists for the selected client" }, { status: 409 });
    if (/UNIQUE constraint failed: employees\.employee_code/i.test(combined)) return Response.json({ error: "This employee code already exists" }, { status: 409 });
    const status = error instanceof RequestError ? error.status : 500;
