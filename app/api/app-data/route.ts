...
import { getTableColumns, and, asc, desc, eq, gte, inArray, lt, sql } from "drizzle-orm";
import { calculateRevenue, type RevenueAssumptions } from "../../../lib/revenue-calculator";
import { calculateRevenueProjection, calculateRevenueScenario, defaultRevenueMonth, indiaToday, monthDates, dateOnly, shiftDate, validateRevenueMonth, type RevenueMonthPlan, type RevenueScenarioInput } from "../../../lib/revenue-projection";
...
import {
  ...
  payrollAttendanceConfirmations,
  auditEvents,
  clientUnits,
  employees,
...
import { mutationCollections } from "../../../lib/mutation-updates";
...
async function requireAppAccess(db: Db): Promise<AppAccess> {
  ...
  const refreshLogin = identity.fullName !== null && identity.fullName !== row.fullName || !row.lastLoginAt || Date.now() - Date.parse(row.lastLoginAt) >= 300000;
  const lastLoginAt = refreshLogin ? new Date().toISOString() : row.lastLoginAt;
  if (refreshLogin) await db.update(appUsers).set({ fullName: identity.fullName ?? row.fullName, lastLoginAt }).where(eq(appUsers.id, row.id));
  return {
    identity,
...
async function loadAppData(access: AppAccess, selection?: string[]) {
  await ensureDemoData();
  const db = getDb();
  const needed = new Set(selection || []);
  const want = (key: string) => !selection || needed.has(key);
  // Identity and organisation rows are required to apply the same scopes as a full read.
  if (selection) {
    for (const key of ["vendors", "units", "employees"]) needed.add(key);
    if (access.profile.role === "hostel_incharge" || want("roomExpenses")) needed.add("accommodationRooms");
    if (selection.some(key => ["payrollItems", "accommodationCharges", "recoveryEntries", "recoveryFinalizations", "payrollBatches", "paymentExportBatches", "paymentExportBatchItems"].includes(key))) needed.add("runs");
    if (want("paymentExportBatchItems")) needed.add("paymentExportBatches");
    if (want("vehicleRecords")) needed.add("vehicles");
    if (want("ebReadings")) needed.add("utilityMeters");
    if (want("hostelUtilityReadings")) needed.add("hostels");
  }
  function loadCollection<T>(key: string, query: PromiseLike<T[]>): Promise<T[]> {
    return want(key) ? Promise.resolve(query) : Promise.resolve([]);
  }
  const [
    allVendorRows,
    allUnitRows,
    ...
    loadCollection("vendors", db.select().from(vendors).orderBy(asc(vendors.name))),
    loadCollection("units", db.select().from(clientUnits).orderBy(asc(clientUnits.clientName))),
    loadCollection("employees", db.select({ ...getTableColumns(employees), ...(selection && !selection.includes("employees") ? { photoDataUrl: sql<string | null>`NULL`, applicationJson: sql<string>`'{}'` } : {}) }).from(employees).orderBy(asc(employees.employeeCode))),
    loadCollection("runs", db.select().from(payrollRuns).orderBy(asc(payrollRuns.payPeriod))),
    loadCollection("payrollItems", db
      .select({
        id: payrollItems.id,
    ...
    loadCollection("accommodationCharges", db
      .select()
      .from(accommodationCharges)
      .orderBy(asc(accommodationCharges.roomNumber))),
    loadCollection("recoveryEntries", db
      .select()
    ...
    loadCollection("recoveryFinalizations", db
      .select()
      .from(recoveryFinalizations)
      .orderBy(desc(recoveryFinalizations.finalizedAt))),
    loadCollection("auditEvents", db
      .select()
      .from(auditEvents)
      .orderBy(desc(auditEvents.createdAt))
      .limit(30)),
    loadCollection("rules", db.select().from(payrollRules).orderBy(asc(payrollRules.vendorId))),
    loadCollection("shifts", db.select().from(shiftDefinitions).orderBy(asc(shiftDefinitions.name))),
    loadCollection("remarks", db
      .select()
      .from(payrollRemarks)
      .orderBy(asc(payrollRemarks.category), asc(payrollRemarks.title))),
    loadCollection("appUsers", canView(access.profile.permissions, "users")
      ? db
          .select()
          .from(appUsers)
          .orderBy(asc(appUsers.fullName), asc(appUsers.email))
      : Promise.resolve([])),
    loadCollection("accommodationTypes", db.select().from(accommodationTypes).orderBy(asc(accommodationTypes.name))),
    loadCollection("accommodationRooms", db
      .select()
      .from(accommodationRooms)
      .orderBy(asc(accommodationRooms.roomNumber))),
    loadCollection("roomExpenses", db
      .select()
      .from(accommodationRoomExpenses)
      .orderBy(desc(accommodationRoomExpenses.payPeriod))),
    loadCollection("payrollBatches", db
      .select()
      .from(payrollBatches)
      .orderBy(asc(payrollBatches.accommodationType))),
    loadCollection("paymentExportBatches", db
      .select()
      .from(paymentExportBatches)
      .orderBy(desc(paymentExportBatches.lockedAt))),
    loadCollection("paymentExportBatchItems", db
      .select()
      .from(paymentExportBatchItems)
      .orderBy(asc(paymentExportBatchItems.createdAt))),
    loadCollection("vehicles", db.select().from(vehicles).orderBy(asc(vehicles.registrationNumber))),
    loadCollection("vehicleRecords", db.select().from(vehicleRecords).orderBy(desc(vehicleRecords.recordDate))),
    loadCollection("utilityMeters", db.select().from(utilityMeters).orderBy(asc(utilityMeters.locationName))),
    loadCollection("ebReadings", db.select().from(ebReadings).orderBy(desc(ebReadings.readingDate))),
    loadCollection("hostels", db.select().from(hostels).orderBy(asc(hostels.name))),
    loadCollection("hostelUtilityReadings", db
      .select()
  ...
  return selection ? Object.fromEntries(Object.entries(full).filter(([key]) => selection.includes(key))) as typeof full : full;
}
...
async function recalculateRun(
  ...
  const [attendanceChoice] = await db.select().from(payrollAttendanceConfirmations).where(eq(payrollAttendanceConfirmations.runId, runId)).limit(1);
  const selectedAttendance = attendanceChoice?.source === "client_attendance"
    ? (await db.select().from(clientAttendanceRecords).where(and(eq(clientAttendanceRecords.runId, runId), gte(clientAttendanceRecords.attendanceDate, start), lt(clientAttendanceRecords.attendanceDate, end)))).map(entry => ({ ...entry, id: 0, source: "client_attendance", punchIn: null, punchOut: null, workedHours: 0, deductionHours: 0, remarks: entry.sourceFile }))
    : periodAttendance;
  const attendanceMap = new Map<string, typeof periodAttendance>();
  for (const entry of selectedAttendance) {
...
async function importWorkbook(
  ...
  if ((await downloadedPayrollItemIds(db, run.id)).size) throw new RequestError("Downloaded bank payments lock client attendance imports for this run.", 409);
  if (sourceType === "salary") {
    await db.insert(payrollAttendanceConfirmations).values({ runId: run.id, status: "pending", clientSalaryImportedAt: new Date().toISOString(), clientFile: optionalValue(payload.sourceFile) ?? optionalValue(payload.sheetName) })
      .onConflictDoUpdate({ target: payrollAttendanceConfirmations.runId, set: { status: "pending", clientSalaryImportedAt: new Date().toISOString(), clientFile: optionalValue(payload.sourceFile) ?? optionalValue(payload.sheetName) } });
  }
  ...
  for (const imported of importedEmployees) {
    ...
    if (!existing && payload.clientAttendance === true) throw new RequestError(`Employee ${code} is not in the employee master. Add and assign the employee before uploading client attendance.`, 409);
  ...
  if (importedAttendance.length) {
    ...
    const stagedClientRows: Array<typeof clientAttendanceRecords.$inferInsert> = [];
    const d1 = getRawDb();
    for (const entry of importedAttendance) {
      ...
      if (payload.clientAttendance === true) {
        stagedClientRows.push({ id: crypto.randomUUID(), runId: run.id, employeeId: employee.id, attendanceDate: date, statusCode: status, shiftCode: shift, overtimeHours: overtime, sourceFile: optionalValue(payload.sourceFile) ?? optionalValue(payload.sheetName), updatedBy: actorEmail, updatedAt: new Date().toISOString() });
        continue;
      }
    ...
    // Keep the whole factory upload atomic and hold the payroll row until all chunks commit.
    if (stagedClientRows.length) {
      if (new Set(stagedClientRows.map(row => `${row.employeeId}|${row.attendanceDate}`)).size !== stagedClientRows.length) throw new RequestError("Client attendance contains duplicate employee/date rows. Correct the file before uploading.");
      await db.transaction(async transaction => {
        const editable = await transaction.update(payrollRuns).set({ updatedAt: new Date().toISOString() }).where(and(
          eq(payrollRuns.id, run.id), sql`${payrollRuns.status} <> 'approved'`,
          sql`NOT EXISTS (SELECT 1 FROM payment_export_batches WHERE run_id = ${run.id} AND status = 'downloaded')`,
        )).returning({ id: payrollRuns.id });
        if (!editable.length) throw new RequestError("Payroll was approved or paid during the upload. Client attendance was not changed.", 409);
        for (let index = 0; index < stagedClientRows.length; index += 500) {
          await transaction.insert(clientAttendanceRecords).values(stagedClientRows.slice(index, index + 500)).onConflictDoUpdate({
            target: [clientAttendanceRecords.runId, clientAttendanceRecords.employeeId, clientAttendanceRecords.attendanceDate],
            set: { statusCode: sql`excluded.status_code`, shiftCode: sql`excluded.shift_code`, overtimeHours: sql`excluded.overtime_hours`, sourceFile: sql`excluded.source_file`, updatedBy: sql`excluded.updated_by`, updatedAt: sql`excluded.updated_at` },
          });
        }
      });
    }
...
export async function POST(request: Request) {
  try {
    ...
    } else if (action === "import-workbook" && payload.clientAttendance === true && payload.sourceType === "attendance") {
      if (!canManage(access.profile.permissions, "attendance") && !canManage(access.profile.permissions, "payroll")) throw new RequestError("Attendance or payroll management access is required for client attendance uploads.", 403);
      if (Array.isArray(payload.salaryItems) && payload.salaryItems.length) throw new RequestError("Client attendance uploads may contain daily attendance only.");
    ...
    if (["get-revenue-projection", "save-revenue-month-plan", "calculate-revenue-projection"].includes(action)) {
      if (access.profile.role !== "super_admin") throw new RequestError("Only Super Admin can access revenue projections.", 403);
      try {
        const month = typeof payload.month === "string" ? payload.month : "";
        const dates = monthDates(month);
        if (action === "save-revenue-month-plan") {
          const unitId = textValue(payload.unitId, "Client unit");
          const [unit] = await db.select({ id: clientUnits.id }).from(clientUnits).where(eq(clientUnits.id, unitId)).limit(1);
          if (!unit) throw new RequestError("Client unit not found.", 404);
          const config = validateRevenueMonth(month, payload.config);
          if (typeof payload.confirmed !== "boolean") throw new RequestError("Choose whether this month's calendar and rates are confirmed.");
          const revision = payload.revision;
          if (typeof revision !== "number" || !Number.isInteger(revision) || revision < 0) throw new RequestError("Reload the monthly setup before saving.");
          const now = new Date().toISOString();
          const values = { id: `REV-${month}-${unitId}`, month, clientUnitId: unitId, configJson: JSON.stringify(config), revision: 1, confirmedAt: payload.confirmed ? now : null, updatedAt: now, updatedBy: access.identity.email };
          const saved = await db.insert(revenueMonthPlans).values(values).onConflictDoUpdate({
            target: [revenueMonthPlans.month, revenueMonthPlans.clientUnitId],
            set: { configJson: values.configJson, confirmedAt: values.confirmedAt, updatedAt: now, updatedBy: values.updatedBy, revision: sql`${revenueMonthPlans.revision} + 1` },
            setWhere: eq(revenueMonthPlans.revision, revision),
          }).returning();
          if (!saved.length) throw new RequestError("This monthly setup was changed in another session. Reload before saving your changes.", 409);
          await writeAudit(db, action, "revenue_month_plan", values.id, `${month} calendar and billing setup ${payload.confirmed ? "confirmed" : "saved as draft"}; revision ${saved[0].revision}`, access.identity.email);
          return Response.json({ plan: { unitId, month, config, revision: saved[0].revision, updatedAt: now, confirmedAt: values.confirmedAt } });
        }
        const [companyRows, unitRows, planRows] = await Promise.all([
          db.select({ id: vendors.id, name: vendors.name, status: vendors.status }).from(vendors).orderBy(asc(vendors.name)),
          db.select({ id: clientUnits.id, vendorId: clientUnits.vendorId, clientName: clientUnits.clientName, unitName: clientUnits.unitName, status: clientUnits.status, attendanceWorkingDays: clientUnits.attendanceWorkingDays, overtimeMultiplier: clientUnits.overtimeMultiplier }).from(clientUnits).orderBy(asc(clientUnits.clientName)),
          db.select().from(revenueMonthPlans).where(eq(revenueMonthPlans.month, month)),
        ]);
        const plans: RevenueMonthPlan[] = planRows.map(p => ({ unitId: p.clientUnitId, month: p.month, config: JSON.parse(p.configJson), revision: p.revision, updatedAt: p.updatedAt, confirmedAt: p.confirmedAt }));
        if (action === "get-revenue-projection") {
          const rules = await db.select().from(payrollRules).orderBy(desc(payrollRules.effectiveFrom));
          return Response.json({ month, today: indiaToday(), companies: companyRows, units: unitRows, plans, defaults: Object.fromEntries(unitRows.map(u => [u.id, defaultRevenueMonth(month, u, rules.find(r => r.vendorId === u.vendorId))])) });
        }
        const cutoff = dateOnly(payload.asOfDate);
        if (cutoff > indiaToday() || cutoff > dates[dates.length - 1]) throw new RequestError("Choose an actuals cutoff no later than today or month end.");
        const units = unitRows.filter(u => (!payload.vendorId || u.vendorId === payload.vendorId) && (!payload.clientName || u.clientName === payload.clientName) && (!payload.unitId || u.id === payload.unitId));
        const ids = units.map(u => u.id);
        const staff = ids.length ? await db.select({ id: employees.id, vendorId: employees.vendorId, clientUnitId: employees.clientUnitId, dateOfJoining: employees.dateOfJoining, dateOfLeaving: employees.dateOfLeaving, status: employees.status, employmentType: employees.employmentType, salaryAmount: employees.salaryAmount, salaryBasis: employees.salaryBasis }).from(employees).where(inArray(employees.clientUnitId, ids)) : [];
        const start = [dates[0], shiftDate(cutoff, -27)].sort()[0];
        const entries = ids.length ? await db.select({ employeeId: attendanceEntries.employeeId, attendanceDate: attendanceEntries.attendanceDate, statusCode: attendanceEntries.statusCode, overtimeHours: attendanceEntries.overtimeHours })
          .from(attendanceEntries).innerJoin(employees, eq(employees.id, attendanceEntries.employeeId))
          .where(and(inArray(employees.clientUnitId, ids), gte(attendanceEntries.attendanceDate, start), lt(attendanceEntries.attendanceDate, shiftDate(cutoff, 1)))) : [];
        const source = { companies: companyRows, units: units.filter(u => u.status === "active" || Boolean(payload.unitId) || staff.some(e => e.clientUnitId === u.id && e.dateOfJoining <= dates[dates.length - 1] && (!e.dateOfLeaving || e.dateOfLeaving >= dates[0]))), employees: staff, attendance: entries, plans };
        const result = calculateRevenueProjection(access.profile.role, source, month, cutoff);
        if (payload.scenario && result.ready) result.scenario = calculateRevenueScenario(access.profile.role, source, result, payload.scenario as RevenueScenarioInput);
        return Response.json({ result });
      } catch (error) {
        if (error instanceof RequestError) throw error;
        throw new RequestError(error instanceof Error ? error.message : "Unable to calculate the revenue projection.");
      }
    }
    if (action === "calculate-revenue") {
      if (access.profile.role !== "super_admin") throw new RequestError("Only Super Admin can use the revenue calculator.", 403);
      const run = await requireRun(db, textValue(payload.runId, "Payroll run"));
      const rows = await db.select({ employeeId: payrollItems.employeeId, grossEarnings: payrollItems.grossEarnings, payableDays: payrollItems.payableDays, overtimeHours: payrollItems.overtimeHours }).from(payrollItems).where(eq(payrollItems.runId, run.id));
      try {
        const result = calculateRevenue(access.profile.role, rows, payload.assumptions as RevenueAssumptions);
        return Response.json({ result, runId: run.id, payPeriod: run.payPeriod, status: run.status });
      } catch (error) { throw new RequestError(error instanceof Error ? error.message : "Invalid calculator assumptions"); }
    }
    ...
    if (action === "save-app-user") {
      ...
      // Store the canonical direct-employee identity, never a caller-supplied display name.
      const application = (() => { try { return JSON.parse(values.applicationJson || "{}"); } catch { return {}; } })();
      if (application && typeof application === "object" && application["Reference 1 - Employee ID"]) {
        const referenceId = String(application["Reference 1 - Employee ID"]);
        const [reference] = await db.select({ id: employees.id, employeeCode: employees.employeeCode, name: employees.name, employmentType: employees.employmentType, status: employees.status }).from(employees).where(eq(employees.id, referenceId)).limit(1);
        if (!reference || reference.employmentType !== "direct" || referenceId === optionalValue(form.id)) throw new RequestError("Reference 1 must be a Joy direct employee other than the applicant.");
        if (reference.status !== "active") {
          const [previous] = form.id ? await db.select({ applicationJson: employees.applicationJson }).from(employees).where(eq(employees.id, String(form.id))).limit(1) : [];
          let previousReference = ""; try { previousReference = JSON.parse(previous?.applicationJson || "{}")["Reference 1 - Employee ID"]; } catch {}
          if (previousReference !== referenceId) throw new RequestError("Select an active direct employee as Reference 1.");
        }
        application["Reference 1 - Employee Code"] = reference.employeeCode;
        application["Reference 1 - Name"] = reference.name;
        values.applicationJson = JSON.stringify(application);
      }
      ...
      const [attendanceConfirmation] = await db.select().from(payrollAttendanceConfirmations).where(eq(payrollAttendanceConfirmations.runId, runId)).limit(1);
      if (attendanceConfirmation?.status !== "confirmed") throw new RequestError("Confirm the final system or client attendance source in Month-end attendance before approving salary.", 409);
    ...
    const selection = payload.responseMode === "delta" ? mutationCollections(action, payload) : undefined;
    const update = await loadAppData(access, selection);
    return Response.json({ ...update, ...(selection ? {delta: true} : {}), selectedRunId: runId }, {
      headers: { "server-timing": `write;dur=${(savedAt-requestStarted).toFixed(1)}, refresh;dur=${(performance.now()-savedAt).toFixed(1)}` },
    });
  } catch (error) {
    const messages = errorMessages(error);
