...
import { applicableFields, type ApplicationDocument } from "../lib/employee-application";
import { onboardingRequest } from "./employee-onboarding";
import type { ClientUnit, Employee, Vendor } from "./payroll-app";

export type NewApplicant = { documents?: ApplicationDocument[]; id: string; name: string; email_address: string; vendor_id: string; client_unit_id: string; employment_type: string; application_json: Record<string, string>; submitted_at: string | null; expires_at: string };
export function FreshOnboarding({ vendors, units, vendorId: initialVendor, unitId: initialUnit, endpoint, publishableKey, accessToken, onReview }: { vendors: Vendor[]; units: ClientUnit[]; vendorId: string; unitId: string; endpoint: string; publishableKey?: string; accessToken?: string; onReview: (applicant: NewApplicant) => void }) {
  ...
  return <section><p>Invite someone who is not yet in Employee Master. Only their email and assignment are required now.</p>
    ...
    {applications.map(a => <div key={a.id} style={{ borderBottom: "1px solid #ddd", padding: "16px 0" }}><strong>{a.name || a.email_address}</strong><p>{a.email_address} · {a.employment_type === "direct" ? "Joy Direct" : "Client location"} · {a.submitted_at ? "Submitted — HR review" : Date.parse(a.expires_at) < Date.now() ? "Expired — enter email above to resend" : "Awaiting employee"}</p>{a.submitted_at && <button className="secondary-button" disabled={busy} onClick={async () => { setBusy(true); setError(""); try { const result = await onboardingRequest(endpoint, publishableKey, { action: "documents", employeeId: a.id }, accessToken); setReview({ ...a, documents: result.documents || [] }); } catch (e) { setError(e instanceof Error ? e.message : "Unable to load documents"); } finally { setBusy(false); } }}>Review application</button>}</div>)}
    {review && <section><h3>{review.name} — submitted details</h3>{Object.entries(applicableFields(review.application_json)).filter(([key,v]) => v && !["Children count", "Sibling count", "Employment records", "Reference 1 - Employee ID"].includes(key)).map(([key, value]) => <p key={key}><strong>{key}:</strong> {value}</p>)}{Boolean(review.documents?.length) && <div><h4>Uploaded documents</h4>{review.documents!.map(d => <p key={d.category}><a href={d.dataUrl} download={d.filename}>{d.category} · {d.filename}</a></p>)}</div>}<p>Review these details, then complete the employee code, joining date, salary, bank and statutory fields before saving.</p><button className="primary-button" onClick={() => onReview(review)}>Open Employee Master with these details</button></section>}
  </section>;
}
