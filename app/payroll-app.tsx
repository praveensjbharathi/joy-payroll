      ...
      <div className="main-column">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <div className="user-chip"><span>{initials(displayName) || "PA"}</span><div><strong>{displayName}</strong><small>{ROLE_LABELS[data.currentUser.role]}</small></div></div>
        ...
        <main className="content">
          ...
          {activeSection === "recoveries" ? <RecoveryCenter run={currentRun} employees={currentEmployees} items={currentItems} charges={data.accommodationCharges} data={data} canManage={mayManage("recoveries") && currentRun?.status !== "approved"} onEdit={(employee, charge) => setModal({ kind: "accommodation", employee, charge })} /> : null}
...
function PaymentsView({ run, items, employees, canExport, onPayslip }: { run: PayrollRun; items: PayrollItem[]; employees: Employee[]; canExport: boolean; onPayslip: (item: PayrollItem) => void }) {
...
function PayslipModal({ item, vendor, unit, run, period, canExport, onClose }: { item: PayrollItem; vendor: Vendor; unit: ClientUnit; run: PayrollRun | null; period: string; canExport: boolean; onClose: () => void }) {
  const selectedEarnings = configuredFields(unit.payslipEarningsJson, earningFields);
  const selectedDeductions = configuredFields(unit.payslipDeductionsJson, deductionFields);
  const earnings = selectedEarnings.map((field) => [readableField(field), Number(item[field as keyof PayrollItem] ?? 0)] as [string, number]);
  const deductions = selectedDeductions.map((field) => [readableField(field), Number(item[field as keyof PayrollItem] ?? 0)] as [string, number]);
