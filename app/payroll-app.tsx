...
export type RecoveryEntry = { id: string; runId: string; employeeId: string; recoveryDate: string; recoveryType: string; amount: number; reference: string | null; notes: string | null; createdBy: string | null; createdAt: string };
export type RecoveryFinalization = { id: string; runId: string; employeeId: string; voucherNumber: string; finalizedBy: string | null; finalizedAt: string };
...
export type Hostel = { id: string; vendorId: string; accommodationTypeId: string | null; name: string; address: string | null; inchargeName: string | null; ebMeterNumber: string | null; clientScopeJson: string; status: string; remarks: string | null };
...
type ActiveModal =
  ...
  | { kind: "run" }
  | { kind: "attendance"; employee: Employee; date: string; entry?: AttendanceEntry }
...
const sectionTitles: Record<Section, { eyebrow: string; title: string; description: string }> = {
  ...
  recoveries: { eyebrow: "Salary recovery control", title: "Employee & room-wise recoveries", description: "Maintain individual deductions separately from shared Gas, Ration and Provision room deductions." },
  ...
  operations: { eyebrow: "Administration controls", title: "Vehicle monitoring", description: "Trips, fuel, mileage, compliance reminders, service follow-ups and vehicle expenses." },
  reports: { eyebrow: "Management information", title: "Reports & downloads", description: "Payroll, attendance, employee, compliance, recovery, hostel and vehicle reports in Excel, CSV and PDF." },
...
const iconPaths: Record<string, string> = {
  ...
  calculator: "M6 3h12a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2Zm2 4h8M8 12h.01M12 12h.01M16 12h.01M8 16h.01M12 16h.01M16 16h.01",
...
function compactMoney(value: number) {
  return new Intl.NumberFormat("en-IN", { notation: "compact", style: "currency", currency: "INR", maximumFractionDigits: 2 }).format(value);
}
...
export default function PayrollApp({
  ...
  useEffect(() => {
    fetch(apiEndpoint, {
      ...
      .then((payload) => {
        setData(payload);
        const firstAllowedSection = navItems.find((item) => canView(payload.currentUser.permissions, sectionPermission[item.id]))?.id ?? "attendance";
        setActiveSection(firstAllowedSection);
        const firstRun = payload.runs.findLast((run) => payload.vendors.some((client) => client.id === run.vendorId && client.status === "active") && payload.units.some((unit) => unit.id === run.clientUnitId && unit.status === "active"));
        const firstClient = payload.vendors.find((client) => client.status === "active");
        const firstUnit = payload.units.find((unit) => unit.vendorId === firstClient?.id && unit.status === "active");
        setActiveVendorId(firstRun?.vendorId ?? firstClient?.id ?? "");
        setActiveUnitId(firstRun?.clientUnitId ?? firstUnit?.id ?? "");
        setActivePeriod(firstRun?.payPeriod ?? new Date().toISOString().slice(0, 7));
      })
      .catch((error) => setAccessError(error instanceof Error ? error.message : "Unable to load application"));
  ...
  const vendorUnits = useMemo(
    () => data?.units.filter((unit) => unit.vendorId === activeVendorId && unit.status === "active") ?? [],
    [data, activeVendorId],
  );
  const currentUnit = data?.units.find((unit) => unit.id === activeUnitId) ?? null;
  const currentVendor = data?.vendors.find((vendor) => vendor.id === activeVendorId) ?? null;
  const unitRuns = data?.runs.filter((run) => run.clientUnitId === activeUnitId).sort((left, right) => right.payPeriod.localeCompare(left.payPeriod)) ?? [];
  const currentRun = unitRuns.find((run) => run.payPeriod === activePeriod) ?? unitRuns[0] ?? null;
  const currentItems = currentRun ? data?.payrollItems.filter((item) => item.runId === currentRun.id) ?? [] : [];
  const currentEmployees = data?.employees.filter((employee) => employee.clientUnitId === activeUnitId) ?? [];
  const operationalEmployees = currentEmployees.filter((employee) => employee.status === "active");
  ...
  const mayManage = (section: Section) => Boolean(data && canManage(data.currentUser.permissions, sectionPermission[section]));

  async function performAction(action: string, successMessage: string, details: Record<string, unknown> = {}) {
    ...
    try {
      const response = await fetch(apiEndpoint, {
        ...
        body: JSON.stringify({ action, runId: currentRun?.id, vendorId: activeVendorId, unitId: activeUnitId, ...details }),
      });
      ...
      if (["create-vendor", "save-client"].includes(action) && !details.id) {
        const vendor = payload.vendors.find((item) => item.code === String(details.code ?? "").toUpperCase());
        if (vendor) { setActiveVendorId(vendor.id); setActiveUnitId(""); }
      }
      if (["create-unit", "save-unit"].includes(action) && !details.id) {
        const unit = payload.units.find((item) => item.vendorId === details.vendorId && item.clientName === details.clientName && item.unitName === details.unitName);
        if (unit) { setActiveVendorId(unit.vendorId); setActiveUnitId(unit.id); }
      }
      if (["set-record-status", "delete-record"].includes(action)) {
        const currentClient = payload.vendors.find((client) => client.id === activeVendorId && client.status === "active");
        const nextClient = currentClient ?? payload.vendors.find((client) => client.status === "active");
        const currentEmployer = payload.units.find((unit) => unit.id === activeUnitId && unit.vendorId === nextClient?.id && unit.status === "active");
        const nextEmployer = currentEmployer ?? payload.units.find((unit) => unit.vendorId === nextClient?.id && unit.status === "active");
  ...
  function selectVendor(vendorId: string) {
    ...
    const firstUnit = data?.units.find((unit) => unit.vendorId === vendorId && unit.status === "active");
    setActiveUnitId(firstUnit?.id ?? "");
  }

  async function updateRecordStatus(entityType: string, entityId: string, currentStatus: string, label: string) {
    const status = currentStatus === "active" ? "inactive" : "active";
    if (status === "inactive" && !window.confirm(`Make ${label} inactive? Existing payroll history will be preserved.`)) return;
    await performAction("set-record-status", `${label} ${status === "active" ? "reactivated" : "made inactive"}`, { entityType, entityId, status });
  }

  async function deleteRecord(entityType: string, entityId: string, label: string) {
    if (!window.confirm(`Permanently delete ${label}? This cannot be undone. Approved payroll history cannot be deleted.`)) return;
    await performAction("delete-record", `${label} deleted`, { entityType, entityId });
  }

  if (!data && accessError) {
    return <main className="access-denied-page"><div className="access-denied-card"><span><Icon name="alert" size={28} /></span><h1>Payroll access unavailable</h1><p>{accessError}</p><small>Ask your Super Admin to add or reactivate your email under Users & Access.</small>{onSignOut ? <button className="secondary-button" onClick={() => void onSignOut()}>Sign out and use another account</button> : null}</div></main>;
  }

  if (!data) {
    return (
      <main className="loading-page">
        <div className="loading-mark"><span>J</span></div>
  ...
  if (!visibleNavItems.length) {
    return <main className="access-denied-page"><div className="access-denied-card"><span><Icon name="alert" size={28} /></span><h1>No modules assigned</h1><p>Your payroll profile is active, but every module is set to No access.</p><small>Ask your Super Admin to enable at least one module under Users & Access.</small>{onSignOut ? <button className="secondary-button" onClick={() => void onSignOut()}>Sign out and use another account</button> : null}</div></main>;
  }
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNavOpen ? "sidebar-open" : ""}`}>
        <div className="brand-lockup">
          ...
          <div><strong>JOY</strong><small>Payroll Manager</small></div>
        </div>
        ...
        <nav aria-label="Application navigation">
          {visibleNavItems.map((item) => (
            <button
              key={item.id}
              className={activeSection === item.id ? "nav-item nav-active" : "nav-item"}
              onClick={() => { setActiveSection(item.id); setMobileNavOpen(false); }}
            >
              <Icon name={item.icon} size={19} />
              <span>{item.label}</span>
              {item.id === "payroll" && currentRun?.issueCount ? <b>{currentRun.issueCount}</b> : null}
            </button>
          ))}
        </nav>
        <div className="sidebar-help">
          <span className="help-icon">?</span>
          <div><strong>Need a payroll check?</strong><small>Review exceptions before approval.</small></div>
        </div>
        <div className="sidebar-footer"><span>joycorporatesolutions.com</span><span>v1.0</span></div>
      </aside>
      ...
      <div className="main-column">
        <header className="topbar">
          ...
          <div className="topbar-selectors">
            ...
            {unitRuns.length ? <><span className="selector-divider period-divider" /><label className="period-selector"><span>Payroll month</span><select value={currentRun?.payPeriod ?? ""} onChange={(event) => setActivePeriod(event.target.value)}>{unitRuns.map((run) => <option key={run.id} value={run.payPeriod}>{monthLabel(run.payPeriod)}</option>)}</select></label></> : null}
          </div>
          <div className="topbar-actions">
            {mayView("payroll") ? <button className="icon-button" aria-label="Payroll issues" onClick={() => setActiveSection("payroll")}><Icon name="alert" /><i>{currentRun?.issueCount ?? 0}</i></button> : null}
            <button className="user-chip user-chip-button" onClick={() => setProfileOpen(true)}><span>{initials(data.currentUser.fullName ?? displayName) || "PA"}</span><div><strong>{data.currentUser.fullName ?? displayName}</strong><small>{ROLE_LABELS[data.currentUser.role]} · Edit profile</small></div></button>
            {onSignOut ? <button className="secondary-button" onClick={() => void onSignOut()}>Sign out</button> : null}
          </div>
        </header>
        ...
        <main className="content">
          <section className="page-heading">
            ...
            <div className="heading-actions">{(mayManage("attendance") || mayManage("payroll") || mayManage("employees")) && !["users", "vendors", "masters", "settings"].includes(activeSection) ? <button className="secondary-button" onClick={() => setModal({ kind: "import" })} disabled={!currentUnit}><Icon name="file" size={16} />Import Excel</button> : null}{mayManage("payroll") && !["users", "vendors", "masters", "settings"].includes(activeSection) ? <button className="period-chip period-button" onClick={() => setModal({ kind: "run" })} disabled={!currentUnit}><Icon name="calendar" size={17} /><span>{currentRun ? `${monthLabel(currentRun.payPeriod)} · New month` : "Create payroll month"}</span></button> : null}</div>
          </section>
          ...
          {!currentRun && !["dashboard", "accommodation", "recoveries", "hostels", "reports", "vendors", "settings", "employees", "masters", "users"].includes(activeSection) ? (
          ...
          {activeSection === "reports" ? <ReportsCenter data={data} run={currentRun} items={currentItems} vendorId={activeVendorId} unitId={activeUnitId} /> : null}
      ...
      {bulkPayslipsOpen && currentVendor && currentUnit && currentRun ? <BulkPayslipModal items={currentItems} vendor={currentVendor} unit={currentUnit} run={currentRun} onClose={() => setBulkPayslipsOpen(false)} /> : null}
      {profileOpen ? <ProfileModal profile={data.currentUser} busy={isActing} onClose={() => setProfileOpen(false)} onSave={async (details) => { const ok = await performAction("save-own-profile", "Profile details updated", details); if (ok) setProfileOpen(false); }} onChangePassword={onChangePassword} /> : null}
...
function Dashboard({ run, items, employees, data, onNavigate }: { run: PayrollRun; items: PayrollItem[]; employees: Employee[]; data: AppData; onNavigate: (section: Section) => void }) {
  const issueItems = items.filter((item) => item.validationStatus !== "ready");
  const paymentBankPercent = run.netPayable ? Math.round((run.bankPayable / run.netPayable) * 100) : 0;
  const departmentCounts = Array.from(new Set(items.map((item) => item.department))).map((department) => ({
    department,
    count: items.filter((item) => item.department === department).length,
    amount: items.filter((item) => item.department === department).reduce((sum, item) => sum + item.netPayable, 0),
  }));
  const maxAmount = Math.max(...departmentCounts.map((row) => row.amount), 1);
  const workflow = [
    { label: "Attendance", detail: `${items.reduce((sum, item) => sum + item.presentDays, 0)} present days`, state: "complete" },
    { label: "Salary calculated", detail: `${items.length} employees`, state: "complete" },
    { label: "Validation", detail: run.issueCount ? `${run.issueCount} issues` : "All checks passed", state: run.issueCount ? "attention" : "complete" },
    { label: "Approval", detail: run.status === "approved" ? "Approved" : "Pending", state: run.status === "approved" ? "complete" : "pending" },
    { label: "Payment", detail: run.status === "approved" ? "Ready to export" : "Locked", state: run.status === "approved" ? "ready" : "pending" },
  ];

  return (
    <div className="section-stack">
      <section className="kpi-grid">
        <MetricCard label="Employees in run" value={String(run.employeeCount)} note={`${employees.filter((employee) => employee.paymentMode === "bank").length} bank · ${employees.filter((employee) => employee.paymentMode === "cash").length} cash`} tone="blue" icon="users" />
        <MetricCard label="Gross earnings" value={compactMoney(run.grossEarnings)} note="Before all deductions" tone="violet" icon="calculator" />
        <MetricCard label="Total deductions" value={compactMoney(run.statutoryDeductions + run.otherDeductions + run.accommodationDeductions)} note={`${compactMoney(run.statutoryDeductions)} statutory`} tone="amber" icon="file" />
        <MetricCard label="Net payable" value={compactMoney(run.netPayable)} note={`${paymentBankPercent}% through bank`} tone="green" icon="bank" />
        <MetricCard label="Exceptions" value={String(run.issueCount)} note={run.issueCount ? "Resolve before approval" : "Payroll is validation-ready"} tone={run.issueCount ? "red" : "green"} icon="alert" />
      </section>

      <section className="panel workflow-panel">
        <div className="panel-heading"><div><span className="eyebrow">Month-end control</span><h2>Payroll readiness</h2></div><button className="text-button" onClick={() => onNavigate("payroll")}>Open payroll run <Icon name="arrow" size={16} /></button></div>
        <div className="workflow-line">
          {workflow.map((step, index) => (
            <div className={`workflow-step workflow-${step.state}`} key={step.label}>
              <div className="workflow-marker">{step.state === "complete" ? <Icon name="check" size={15} /> : index + 1}</div>
      ...
      <div className="dashboard-grid">
        <section className="panel">
          <div className="panel-heading"><div><span className="eyebrow">Net payroll mix</span><h2>Department view</h2></div><span className="muted-label">{money(run.netPayable)} total</span></div>
          <div className="bar-list">
            {departmentCounts.map((row) => (
              <div className="bar-row" key={row.department}>
                <div><strong>{row.department}</strong><span>{row.count} employee{row.count === 1 ? "" : "s"}</span></div>
                <div className="bar-track"><i style={{ width: `${Math.max(10, (row.amount / maxAmount) * 100)}%` }} /></div>
        ...
        <section className="panel payment-mix-panel">
          <div className="panel-heading"><div><span className="eyebrow">Payment mode</span><h2>Bank vs cash</h2></div></div>
          <div className="payment-mix">
            <div className="donut" style={{ background: `conic-gradient(#2677f2 0 ${paymentBankPercent}%, #f2a94b ${paymentBankPercent}% 100%)` }}><span><strong>{paymentBankPercent}%</strong><small>bank pay</small></span></div>
            <div className="mix-legend">
              <div><i className="legend-bank" /><span><strong>{money(run.bankPayable)}</strong><small>Bank transfer</small></span></div>
              <div><i className="legend-cash" /><span><strong>{money(run.cashPayable)}</strong><small>Cash payable</small></span></div>
            </div>
          </div>
          <button className="secondary-button full-button" onClick={() => onNavigate("payments")}>Prepare payment outputs</button>
        </section>
      </div>

      <section className="panel">
        <div className="panel-heading"><div><span className="eyebrow">Validation queue</span><h2>Exceptions requiring action</h2></div><button className="text-button" onClick={() => onNavigate("payroll")}>{issueItems.length ? "Review all" : "View payroll"} <Icon name="arrow" size={16} /></button></div>
        {issueItems.length ? (
          <div className="exception-list">
            {issueItems.map((item) => (
              <div className="exception-row" key={item.id}>
                <span className="employee-avatar">{initials(item.employeeName)}</span>
                <div><strong>{item.employeeName}</strong><small>{item.employeeCode} · {item.department}</small></div>
                <span className="exception-message"><Icon name="alert" size={16} />{item.validationMessage}</span>
                <b>{money(item.netPayable)}</b>
              </div>
            ))}
          </div>
        ) : <EmptyState icon="check" title="No payroll exceptions" detail="All employee and calculation checks are ready for approval." />}
      </section>

      {data.auditEvents.length ? <section className="panel compact-panel"><div className="panel-heading"><div><span className="eyebrow">Control history</span><h2>Recent activity</h2></div></div><div className="activity-row">{data.auditEvents.slice(-3).reverse().map((event) => <div key={event.id}><span><Icon name="check" size={15} /></span><p><strong>{event.summary}</strong><small>{event.actorEmail ?? "Payroll system"}</small></p></div>)}</div></section> : null}
    </div>
  );
}

function MetricCard({ label, value, note, tone, icon }: { label: string; value: string; note: string; tone: string; icon: string }) {
  return <article className={`metric-card metric-${tone}`}><div className="metric-top"><span>{label}</span><i><Icon name={icon} size={18} /></i></div><strong>{value}</strong><small>{note}</small></article>;
}

function PayrollRunView({ run, items, isActing, canManage, canManageEmployees, canApprove, onAction, onSelect, onEmployees }: { run: PayrollRun; items: PayrollItem[]; isActing: boolean; canManage: boolean; canManageEmployees: boolean; canApprove: boolean; onAction: (action: string, message: string) => Promise<boolean>; onSelect: (item: PayrollItem) => void; onEmployees: () => void }) {
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState("all");
  const visible = items.filter((item) => {
    const matchesQuery = `${item.employeeCode} ${item.employeeName} ${item.department}`.toLowerCase().includes(query.toLowerCase());
    const matchesFilter = filter === "all" || (filter === "review" ? item.validationStatus !== "ready" : item.validationStatus === "ready");
    return matchesQuery && matchesFilter;
  });
  function exportPayroll() {
    downloadCsv(`payroll-register-${run.payPeriod}.csv`, [
      ["Emp ID", "Employee", "Department", "Present", "Absent", "Leave", "WO/H", "Holiday Present", "Payable Days", "OT Hours", "Gross Earnings", "Salary Deductions", "Accommodation Deductions", "Return Amount", "Net Payable", "Payment Mode", "Validation"],
      ...visible.map((item) => [item.employeeCode, item.employeeName, item.department, item.presentDays, item.absentDays, item.leaveDays, item.weekOffDays, item.holidayPresentDays, item.payableDays, item.overtimeHours, item.grossEarnings, item.totalDeductions - item.accommodationDeduction, item.accommodationDeduction, item.returnAmount, item.netPayable, item.paymentMode, item.validationStatus]),
    ]);
  }
  return (
    <div className="section-stack">
      <section className={`run-command ${run.status === "approved" ? "command-approved" : run.issueCount ? "command-attention" : "command-ready"}`}>
        <div className="command-icon">{run.status === "approved" ? <Icon name="check" /> : <Icon name={run.issueCount ? "alert" : "calculator"} />}</div>
        <div><span className="eyebrow">Run {run.id} · {run.processingMode === "salary_import" ? "Imported salary source" : "Application attendance source"}</span><h2>{run.status === "approved" ? "Payroll approved" : !items.length ? "Add employees to start payroll" : run.issueCount ? `${run.issueCount} employee records need review` : run.grossEarnings <= 0 ? "Add attendance or salary amounts" : "Payroll passed all validation checks"}</h2><p>{run.status === "approved" ? `Approved by ${run.approvedBy ?? "Payroll approver"}. Payment exports are unlocked.` : !items.length ? "Add or import employees, then record their attendance and salary." : run.issueCount ? "Update employee bank or statutory details, then recheck the run." : run.grossEarnings <= 0 ? "Record attendance or open an employee salary row to enter earnings." : run.processingMode === "salary_import" ? "Imported salary-slip headers are authoritative; review recoveries and approve." : "Review the attendance-based calculations, then approve the final salary run."}</p></div>
        <div className="command-actions">
          {canManage ? <>
            {run.status !== "approved" ? <><button className="secondary-button" disabled={isActing} onClick={() => onAction("recalculate", run.processingMode === "salary_import" ? "Imported salary totals recalculated" : "Attendance and payroll totals recalculated")}>{isActing ? <span className="button-spinner" /> : <Icon name="refresh" size={17} />}{run.processingMode === "salary_import" ? "Refresh totals" : "Recalculate attendance"}</button><button className="danger-button" disabled={isActing} onClick={() => { if (window.confirm(`Delete the ${monthLabel(run.payPeriod)} payroll run? Salary calculations and accommodation recoveries will be removed, but attendance history will remain.`)) void onAction("delete-payroll-run", "Payroll run deleted; attendance history preserved"); }}>Delete run</button></> : null}
            {!items.length ? canManageEmployees ? <button className="primary-button" onClick={onEmployees}>+ Add employees</button> : <span className="access-mode-note">HR must add employees</span> : run.issueCount ? <><button className="secondary-button" disabled={isActing} onClick={() => onAction("resolve-issues", "Employee records checked")}>Recheck</button>{canManageEmployees ? <button className="primary-button" onClick={onEmployees}>Edit employee records</button> : null}</> : run.status !== "approved" ? canApprove ? <button className="primary-button" disabled={isActing || run.grossEarnings <= 0} onClick={() => onAction("approve", "Payroll approved and payment outputs unlocked")}><Icon name="check" size={17} />Approve payroll</button> : <span className="access-mode-note"><Icon name="alert" size={15} />Awaiting an authorised approver</span> : <button className="secondary-button" disabled={isActing} onClick={() => onAction("reopen", "Payroll reopened for corrections")}><Icon name="refresh" size={17} />Reopen payroll</button>}
          </> : <span className="access-mode-note"><Icon name="eye" size={15} />View-only payroll access</span>}
        </div>
      </section>

      <section className="panel table-panel">
        <div className="table-toolbar">
          <div className="search-field"><Icon name="search" size={17} /><input aria-label="Search employee" placeholder="Search employee, ID or department" value={query} onChange={(event) => setQuery(event.target.value)} /></div>
          <div className="filter-tabs"><button className={filter === "all" ? "filter-active" : ""} onClick={() => setFilter("all")}>All <span>{items.length}</span></button><button className={filter === "ready" ? "filter-active" : ""} onClick={() => setFilter("ready")}>Ready <span>{items.filter((item) => item.validationStatus === "ready").length}</span></button><button className={filter === "review" ? "filter-active" : ""} onClick={() => setFilter("review")}>Review <span>{items.filter((item) => item.validationStatus !== "ready").length}</span></button></div>
          {canManage ? <button className="secondary-button" onClick={exportPayroll}><Icon name="download" size={16} />Excel-compatible CSV</button> : null}
        </div>
        <div className="table-scroll">
          <table className="data-table payroll-table">
            <thead><tr><th>Employee</th><th>Payable / OT</th><th>Gross earnings</th><th>Salary deductions</th><th>Accommodation</th><th>Net payable</th><th>Validation</th><th /></tr></thead>
            <tbody>{visible.map((item) => <tr key={item.id}><td><EmployeeCell name={item.employeeName} code={item.employeeCode} detail={item.department} /></td><td><strong>{item.payableDays} days</strong><small>{item.overtimeHours} OT hrs</small></td><td>{money(item.grossEarnings)}</td><td>{money(item.totalDeductions - item.accommodationDeduction)}</td><td>{money(item.accommodationDeduction)}<small>{item.accommodationType}</small></td><td className="net-cell">{money(item.netPayable)}</td><td><StatusPill status={item.validationStatus === "ready" ? "Ready" : "Review"} /></td><td><button className="row-action" aria-label={`Review ${item.employeeName}`} onClick={() => onSelect(item)}><Icon name="chevron" size={17} /></button></td></tr>)}</tbody>
            <tfoot><tr><td colSpan={2}><strong>{visible.length} employees</strong></td><td>{money(visible.reduce((sum, item) => sum + item.grossEarnings, 0))}</td><td>{money(visible.reduce((sum, item) => sum + item.totalDeductions - item.accommodationDeduction, 0))}</td><td>{money(visible.reduce((sum, item) => sum + item.accommodationDeduction, 0))}</td><td>{money(visible.reduce((sum, item) => sum + item.netPayable, 0))}</td><td colSpan={2} /></tr></tfoot>
          </table>
        </div>
      </section>
    </div>
...
function AttendanceView({ period, periodStart, periodEnd, items, employees, attendance, canManage, onEdit, onImport, locked }: { period: string; periodStart: string | null; periodEnd: string | null; items: PayrollItem[]; employees: Employee[]; attendance: AttendanceEntry[]; canManage: boolean; onEdit: (employee: Employee, date: string, entry?: AttendanceEntry) => void; onImport: () => void; locked: boolean }) {
  ...
  function exportAttendance() {
    downloadCsv(`attendance-${period}.csv`, [
      ["Emp ID", "Name", "Department", "Default shift", ...dates.map((date) => date.slice(8))],
      ...employees.map((employee) => [employee.employeeCode, employee.name, employee.department, employee.defaultShift, ...dates.map((date) => entryMap.get(`${employee.id}:${date}`)?.statusCode ?? "")]),
    ]);
  }
  return (
    <div className="section-stack">
      <section className="rule-callout"><span><Icon name="calendar" /></span><div><strong>{locked ? "This attendance register is read only." : "Click any date to enter attendance, shift, and overtime."}</strong><p>Shifts can change every day and week offs may occur on any weekday. Import the original Attendance Input workbook to fill the whole month.</p></div></section>
      <section className="kpi-grid attendance-kpis">
        <MetricCard label="Present days" value={String(items.reduce((sum, item) => sum + item.presentDays, 0))} note="Includes holiday present" tone="green" icon="check" />
        <MetricCard label="Absent days" value={String(items.reduce((sum, item) => sum + item.absentDays, 0))} note={`${followups.length} follow-up cases`} tone="red" icon="alert" />
        <MetricCard label="Leave days" value={String(items.reduce((sum, item) => sum + item.leaveDays, 0))} note="Continuous streak tracked" tone="violet" icon="calendar" />
        <MetricCard label="Holiday work" value={String(items.reduce((sum, item) => sum + item.holidayPresentDays, 0))} note="Feeds holiday wages" tone="amber" icon="calculator" />
        <MetricCard label="OT hours" value={String(items.reduce((sum, item) => sum + item.overtimeHours, 0))} note="Feeds overtime wages" tone="blue" icon="calculator" />
      </section>
      <section className="panel table-panel">
        <div className="panel-heading attendance-heading"><div><span className="eyebrow">{monthLabel(period)} daily roster</span><h2>Attendance + shift matrix</h2></div><div className="attendance-toolbar">{canManage ? <><button className="secondary-button" onClick={exportAttendance}><Icon name="download" size={15} />Export CSV</button><button className="primary-button" onClick={onImport} disabled={locked}>Import attendance</button></> : <span className="access-mode-note"><Icon name="eye" size={15} />View only</span>}</div></div>
        <div className="status-legend attendance-legend">{Object.entries(statusMeta).map(([code, meta]) => <span key={code}><i className={meta.className}>{code}</i>{meta.label}</span>)}</div>
        <div className="table-scroll"><table className="data-table attendance-table"><thead><tr><th className="sticky-col">Employee</th>{dates.map((date) => <th key={date}><strong>{new Date(`${date}T00:00:00`).toLocaleDateString("en-IN", { day: "2-digit", month: "short" })}</strong><small>{new Date(`${date}T00:00:00`).toLocaleDateString("en-IN", { weekday: "short" })}</small></th>)}<th>Monthly summary</th></tr></thead><tbody>{employees.map((employee) => { const item = items.find((row) => row.employeeId === employee.id); return <tr key={employee.id}><td className="sticky-col"><EmployeeCell name={employee.name} code={employee.employeeCode} detail={employee.department} /></td>{dates.map((date) => { const entry = entryMap.get(`${employee.id}:${date}`); return <td key={date}><button className="attendance-edit" disabled={locked} title={`${employee.name} · ${date}`} onClick={() => onEdit(employee, date, entry)}>{entry ? <div className="attendance-mark"><b className={statusMeta[entry.statusCode]?.className ?? ""}>{entry.statusCode}</b><span>{entry.shiftCode.replace(" Shift", "")}</span>{entry.overtimeHours ? <em>+{entry.overtimeHours}h</em> : null}</div> : <span className="missing-mark">+</span>}</button></td>; })}<td><div className="attendance-summary"><strong>{item?.presentDays ?? 0} P</strong><span>{item?.absentDays ?? 0} A · {item?.leaveDays ?? 0} L</span></div></td></tr>; })}</tbody></table></div>
        {!employees.length ? <EmptyState icon="users" title="No employees yet" detail="Add employees or import the Attendance Input workbook to begin." /> : null}
      </section>
      <div className="dashboard-grid">
        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">HR follow-up</span><h2>Continuous absent / leave</h2></div></div>{followups.length ? <div className="compact-list">{followups.map((item) => <div key={item.id}><EmployeeCell name={item.employeeName} code={item.employeeCode} detail={item.department} /><span><b>{item.absentDays} A</b><b>{item.leaveDays} L</b></span><StatusPill status={item.absentDays >= 3 ? "High" : "Follow-up"} /></div>)}</div> : <EmptyState icon="check" title="No follow-ups" detail="No continuous absent or leave streaks exceed the threshold." />}</section>
        <section className="panel"><div className="panel-heading"><div><span className="eyebrow">High attendance</span><h2>Maximum working days</h2></div></div><div className="rank-list">{[...items].sort((a, b) => b.presentDays - a.presentDays).slice(0, 5).map((item, index) => <div key={item.id}><span>{index + 1}</span><EmployeeCell name={item.employeeName} code={item.employeeCode} detail={item.department} /><strong>{item.presentDays + item.holidayPresentDays}<small>worked</small></strong></div>)}</div></section>
      </div>
    </div>
  );
}

function EmployeesView({ employees, vendors, units, canManage, onAdd, onEdit, onImport, onLeft, onReactivate, onWorkflow }: { employees: Employee[]; vendors: Vendor[]; units: ClientUnit[]; canManage: boolean; onAdd: () => void; onEdit: (employee: Employee) => void; onImport: () => void; onLeft: (employee: Employee) => void; onReactivate: (employee: Employee) => void; onWorkflow: (employee: Employee) => void }) {
  const [query, setQuery] = useState("");
  ...
  function exportEmployees() {
    downloadCsv("employee-master.csv", [
      ["Emp ID", "Name", "Department", "Date of Joining", "UAN", "ESI", "Bank Account", "IFSC", "Bank Name", "Accommodation Type", "Room", "Payment Mode", "Salary Amount", "Salary Basis", "Default Shift", "Readiness", "Status"],
      ...visible.map((employee) => [employee.employeeCode, employee.name, employee.department, employee.dateOfJoining, employee.uanMasked ?? "", employee.esiMasked ?? "", employee.bankAccountMasked ?? "", employee.ifscMasked ?? "", employee.bankName ?? "", employee.accommodationType, employee.roomNumber ?? "", employee.paymentMode, employee.salaryAmount, employee.salaryBasis, employee.defaultShift, employee.complianceStatus, employee.status]),
    ]);
  }
  return (
    <div className="section-stack">
      ...
      {cardEmployee ? <EmployeeIdCard employee={cardEmployee} vendor={vendors.find((vendor) => vendor.id === cardEmployee.vendorId)} unit={units.find((unit) => unit.id === cardEmployee.clientUnitId)} onClose={() => setCardEmployee(null)} /> : null}
    </div>
  );
}

function EmployeeIdCard({ employee, vendor, unit, onClose }: { employee: Employee; vendor?: Vendor; unit?: ClientUnit; onClose: () => void }) {
  ...
  return <div className="modal-layer"><button className="modal-scrim" onClick={onClose} aria-label="Close ID card" /><div className="id-card-modal"><div className="modal-toolbar"><strong>Employee ID card</strong><div><button className="secondary-button" onClick={() => window.print()}>Print / Save PDF</button><button className="icon-button" onClick={onClose}>×</button></div></div><article className="employee-id-card"><header className="id-card-company">{vendor?.logoDataUrl ? <img src={vendor.logoDataUrl} alt={`${vendor.name} logo`} /> : null}<div><b>{vendor?.legalName ?? "JOY GROUPS"}</b><span>{unit ? `${unit.clientName} · ${unit.location}` : "DIRECT EMPLOYEE"}</span></div></header>{employee.photoDataUrl ? <img className="employee-id-photo-image" src={employee.photoDataUrl} alt={`${employee.name} photo`} /> : <div className="employee-id-photo">{initials(employee.name)}</div>}<h2>{employee.name}</h2><strong>{employee.department}</strong><dl><div><dt>Employee ID</dt><dd>{employee.employeeCode}</dd></div><div><dt>Joining date</dt><dd>{employee.dateOfJoining}</dd></div><div><dt>Group company</dt><dd>{vendor?.name ?? "Joy Group"}</dd></div><div><dt>Client</dt><dd>{unit?.clientName ?? "Direct employee"}</dd></div><div><dt>Employer unit</dt><dd>{unit ? `${unit.unitName} · ${unit.location}` : "Joy Group"}</dd></div><div><dt>Employment</dt><dd>{employee.employmentType === "direct" ? "Direct" : "Client assigned"}</dd></div></dl>{qr ? <img src={qr} alt={`QR code for ${employee.employeeCode}`} /> : null}<small>Scan to verify employee identity details</small></article></div></div>;
}
...
function PaymentsView({ run, items, employees, vendor, unit, canExport, onPayslip, onBulkPayslips }: { run: PayrollRun; items: PayrollItem[]; employees: Employee[]; vendor: Vendor; unit: ClientUnit; canExport: boolean; onPayslip: (item: PayrollItem) => void; onBulkPayslips: () => void }) {
  ...
  function exportRows(mode: "bank" | "cash") {
    ...
    const header = mode === "bank" ? ["SNO_REF_NO", "CUSTOMER_NAME", "ACCOUNT_NO", "AMOUNT", "DESCRIPTION", "IFSC_CODE", "BANK_NAME"] : ["SL_NO", "EMP_ID", "EMPLOYEE_NAME", "AMOUNT", "PAYMENT_MODE"];
    const values = rows.map((item, index) => mode === "bank" ? [index + 1, item.employeeName, item.bankAccountMasked ?? "", item.netPayable, `${monthLabel(run.payPeriod)} Salary`, item.ifscMasked ?? "", employees.find((employee) => employee.id === item.employeeId)?.bankName ?? ""] : [index + 1, item.employeeCode, item.employeeName, item.netPayable, "Cash"]);
    downloadCsv(`${mode}-payment-${run.payPeriod}.csv`, [header, ...values]);
  }
  function exportIndianBank() {
    const rows = bankItems.map((item, index) => { const employee = employees.find((row) => row.id === item.employeeId); return [index + 1, item.employeeName, unit.location, item.bankAccountMasked ?? "", item.netPayable, `Salary ${monthLabel(run.payPeriod)}`, item.ifscMasked ?? "", employee?.bankName ?? "", ""]; });
    downloadXlsx(`indian-bank-bulk-${run.payPeriod}.xlsx`, [["SNO_REF_NO", "CUSTOMER_NAME", "CITY", "ACCOUNT_NO", "AMOUNT", "DESCRIPTION", "IFSC_CODE", "BANK_NAME", "BENEFICIARY_EMAIL_ID"], ...rows]);
  }
  function exportCubAnyBank() {
    const rows = bankItems.filter((item) => !String(item.ifscMasked ?? "").toUpperCase().startsWith("CIUB")).map((item) => `NEFT~${item.ifscMasked ?? ""}~${item.netPayable.toFixed(2)}~10~${item.bankAccountMasked ?? ""}~${vendor.legalName.toUpperCase()}~0~SALARY ${run.payPeriod}`);
    downloadText(`cub-any-bank-${run.payPeriod}.txt`, rows.join("\r\n"));
  }
  function exportCubToCub() {
    const rows = bankItems.filter((item) => String(item.ifscMasked ?? "").toUpperCase().startsWith("CIUB")).map((item) => `${item.bankAccountMasked ?? ""}~${item.netPayable.toFixed(2)}~${vendor.name.toUpperCase()} ${run.payPeriod} SALARY`);
    downloadText(`cub-to-cub-${run.payPeriod}.txt`, rows.join("\r\n"));
  }
  return (
    <div className="section-stack">
      <section className="panel panel-heading"><div><span className="eyebrow">Payslip download centre</span><h2>Bulk payslips — two slips per A4 page</h2><p>Logo source: Group Companies &amp; Clients → Edit group company → Company logo for payslips.</p></div><button className="primary-button" onClick={onBulkPayslips} disabled={!items.length}><Icon name="download" size={16} />Download bulk payslips PDF</button></section>
      <section className="payment-summary-grid"><article><span><Icon name="bank" /></span><div><small>Bank transfer</small><strong>{money(run.bankPayable)}</strong><p>{bankItems.length} employees · final payable after recoveries</p></div><button onClick={() => exportRows("bank")} disabled={run.status !== "approved" || !canExport}><Icon name="download" size={16} />{canExport ? "CUB / bank CSV" : "View only"}</button></article><article><span className="cash-icon">₹</span><div><small>Cash payable</small><strong>{money(run.cashPayable)}</strong><p>{cashItems.length} employees · acknowledgement required</p></div><button onClick={() => exportRows("cash")} disabled={run.status !== "approved" || !canExport}><Icon name="download" size={16} />{canExport ? "Cash list CSV" : "View only"}</button></article><article><span><Icon name="file" /></span><div><small>Half-A4 payslips</small><strong>{items.length}</strong><p>Two employee slips per A4 page</p></div><div className="record-actions"><button onClick={() => items[0] && onPayslip(items[0])} disabled={!items.length}>Preview</button><button onClick={onBulkPayslips} disabled={!items.length}>Bulk PDF</button></div></article></section>
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Bank bulk-upload formats</span><h2>Downloads matching supplied bank templates</h2></div><span className="muted-label">{bankItems.length} bank employees</span></div><div className="record-actions"><button className="secondary-button" onClick={exportIndianBank} disabled={!bankItems.length || run.status !== "approved" || !canExport}>Indian Bank Excel</button><button className="secondary-button" onClick={exportCubAnyBank} disabled={!bankItems.length || run.status !== "approved" || !canExport}>CUB Any Bank TXT</button><button className="secondary-button" onClick={exportCubToCub} disabled={!bankItems.length || run.status !== "approved" || !canExport}>CUB-to-CUB TXT</button></div></section>
      {run.status !== "approved" ? <section className="locked-banner"><span><Icon name="alert" /></span><div><strong>Payment outputs are locked until payroll approval.</strong><p>Resolve all validation issues, recalculate and approve the payroll run first.</p></div></section> : <section className="success-banner"><span><Icon name="check" /></span><div><strong>Payroll is approved for disbursement.</strong><p>Bank, cash and payslip outputs are now available.</p></div></section>}
      <section className="panel table-panel"><div className="panel-heading"><div><span className="eyebrow">Bank validation</span><h2>Payment-ready employee list</h2></div><span className="muted-label">Masked in demo</span></div><div className="table-scroll"><table className="data-table"><thead><tr><th>S.No</th><th>Employee</th><th>Account</th><th>IFSC</th><th>Mode</th><th>Net payable</th><th>Readiness</th><th>Payslip</th></tr></thead><tbody>{items.map((item, index) => <tr key={item.id}><td>{index + 1}</td><td><EmployeeCell name={item.employeeName} code={item.employeeCode} detail={item.department} /></td><td>{item.bankAccountMasked ?? <span className="pending-text">Pending</span>}</td><td>{item.ifscMasked ?? <span className="pending-text">Pending</span>}</td><td><span className="mode-pill">{item.paymentMode === "bank" ? "Bank" : "Cash"}</span></td><td className="net-cell">{money(item.netPayable)}</td><td><StatusPill status={item.validationStatus === "ready" ? "Ready" : "Review"} /></td><td><button className="payslip-button" onClick={() => onPayslip(item)}><Icon name="eye" size={16} />View</button></td></tr>)}</tbody></table></div></section>
    </div>
  );
}

function VendorsView({ vendors, units, activeVendorId, canManage, onSelect, onAddVendor, onEditVendor, onAddUnit, onEditUnit, onVendorStatus, onDeleteVendor, onUnitStatus, onDeleteUnit }: {
  vendors: Vendor[];
  units: ClientUnit[];
  ...
  return (
    <div className="section-stack">
      <section className="vendor-actions"><div><strong>{vendors.filter((vendor) => vendor.status === "active").length} active group companies</strong><span>Joy group companies are maintained separately from their client factories and employer units.</span></div>{canManage ? <button className="primary-button" onClick={onAddVendor}>+ Add group company</button> : <span className="access-mode-note"><Icon name="eye" size={15} />View only</span>}</section>
      <section className="vendor-grid">{vendors.map((vendor) => { const vendorUnits = units.filter((unit) => unit.vendorId === vendor.id); return <article className={vendor.id === activeVendorId ? "vendor-card vendor-selected" : "vendor-card"} key={vendor.id}><header>{vendor.logoDataUrl ? <img className="vendor-logo vendor-logo-image" src={vendor.logoDataUrl} alt={`${vendor.name} logo`} /> : <button className="vendor-logo vendor-logo-upload" onClick={() => onEditVendor(vendor)} title="Upload company logo">+ Logo</button>}<StatusPill status={vendor.status === "active" ? "Active" : "Inactive"} /></header><h2>{vendor.legalName}</h2><p>{vendorUnits.length} client units · {vendorUnits.reduce((sum, unit) => sum + unit.employeeCount, 0)} active employees</p>{vendor.remarks ? <p className="record-remarks">{vendor.remarks}</p> : null}<dl><div><dt>EPF code</dt><dd>{vendor.epfCode ?? "Not added"}</dd></div><div><dt>ESI code</dt><dd>{vendor.esiCode ?? "Not added"}</dd></div><div><dt>GSTIN</dt><dd>{vendor.gstin ?? "Not added"}</dd></div></dl><div className="client-card-footer"><button className="secondary-button" disabled={vendor.status !== "active"} onClick={() => onSelect(vendor.id, vendorUnits.find((unit) => unit.status === "active")?.id ?? "")}>Open group company <Icon name="arrow" size={16} /></button>{canManage ? <RecordActions status={vendor.status} onEdit={() => onEditVendor(vendor)} onToggle={() => onVendorStatus(vendor)} onDelete={() => onDeleteVendor(vendor)} /> : null}</div></article>; })}</section>
      <section className="panel table-panel"><div className="panel-heading"><div><span className="eyebrow">Operational structure</span><h2>Clients and employer units</h2></div>{canManage ? <button className="primary-button" onClick={onAddUnit} disabled={!vendors.some((vendor) => vendor.status === "active")}>+ Add client unit</button> : null}</div><div className="table-scroll"><table className="data-table"><thead><tr><th>Group company</th><th>Client name</th><th>Employer unit</th><th>Location / remarks</th><th>Active employees</th><th>Status</th><th>Open</th>{canManage ? <th>Actions</th> : null}</tr></thead><tbody>{units.map((unit) => { const canOpen = unit.status === "active" && vendors.find((vendor) => vendor.id === unit.vendorId)?.status === "active"; return <tr key={unit.id}><td>{vendors.find((vendor) => vendor.id === unit.vendorId)?.code}</td><td><strong>{unit.clientName}</strong></td><td>{unit.unitName}</td><td><strong>{unit.location}</strong>{unit.remarks ? <small>{unit.remarks}</small> : null}</td><td>{unit.employeeCount}</td><td><StatusPill status={unit.status === "active" ? "Active" : "Inactive"} /></td><td>{canOpen ? <button className="record-action" onClick={() => onSelect(unit.vendorId, unit.id)}>Open</button> : "—"}</td>{canManage ? <td><RecordActions status={unit.status} onEdit={() => onEditUnit(unit)} onToggle={() => onUnitStatus(unit)} onDelete={() => onDeleteUnit(unit)} /></td> : null}</tr>; })}</tbody></table></div>{!units.length ? <EmptyState icon="building" title="No client units yet" detail="Add a group company first, then create its client and employer units." /> : null}</section>
    </div>
  );
}
...
function UsersAccessView({ users, vendors, units, currentUser, canManage, onAdd, onEdit, onStatus, onDelete }: {
  ...
  const roleCounts = (Object.keys(ROLE_LABELS) as UserRole[]).map((role) => ({
    role,
    count: users.filter((profile) => profile.role === role && profile.status === "active").length,
  }));
  ...
  return <div className="section-stack">
    ...
    <section className="access-security-note"><Icon name="alert" size={19} /><div><strong>Temporary-password access protects every new user.</strong><p>The Super Admin creates the login and temporary password here. At the first successful sign-in, the user must immediately set a private password before accessing the application.</p></div></section>
    <section className="panel table-panel">
...
function SettingsView({ vendor, rules, canManage, isActing, onSave }: { vendor: Vendor; rules: PayrollRule | null; canManage: boolean; isActing: boolean; onSave: (rules: Record<string, unknown>) => Promise<boolean> }) {
  const earnings = ["Basic", "DA", "HRA", "Conveyance allowance", "Food allowance", "Night allowance", "OT wages", "Attendance bonus", "Arrears", "Holiday wages", "Production incentive", "Medical allowance"];
  const deductions = ["PF", "ESI", "Professional Tax", "LWF", "Canteen", "Snacks", "Tent / accommodation", "Advance", "Others", "TDS", "Medical insurance"];
  const [form, setForm] = useState({ ...defaultPayrollRules, effectiveFrom: new Date().toISOString().slice(0, 10), ...rules });
  function update(field: string, value: string | number | boolean) { setForm((current) => ({ ...current, [field]: value })); }
  return (
    <div className="section-stack">
      <section className="rule-callout"><span><Icon name="settings" /></span><div><strong>Salary and statutory rules are saved separately for {vendor.name}.</strong><p>Enter only approved PF, ESI, tax, overtime and attendance settings. A zero statutory rate means that component is not automatically calculated.</p></div></section>
      <form className="panel rules-form" onSubmit={(event) => { event.preventDefault(); if (canManage) void onSave(form); }}><div className="panel-heading"><div><span className="eyebrow">Entity-specific controls</span><h2>Calculation rules</h2></div>{canManage ? <button className="primary-button" type="submit" disabled={isActing}>{isActing ? "Saving…" : "Save payroll rules"}</button> : <span className="access-mode-note"><Icon name="eye" size={15} />View only</span>}</div><fieldset className="form-grid rules-grid rules-fieldset" disabled={!canManage}><label><span>Standard working days</span><input type="number" min="1" max="31" value={form.standardWorkingDays} onChange={(event) => update("standardWorkingDays", Number(event.target.value))} required /></label><label><span>PF employee rate (%)</span><input type="number" min="0" max="100" step="0.01" value={form.pfRate} onChange={(event) => update("pfRate", Number(event.target.value))} /></label><label><span>ESI employee rate (%)</span><input type="number" min="0" max="100" step="0.01" value={form.esiRate} onChange={(event) => update("esiRate", Number(event.target.value))} /></label><label><span>Professional tax (₹)</span><input type="number" min="0" value={form.professionalTax} onChange={(event) => update("professionalTax", Number(event.target.value))} /></label><label><span>Labour welfare fund (₹)</span><input type="number" min="0" value={form.lwf} onChange={(event) => update("lwf", Number(event.target.value))} /></label><label><span>Overtime hourly rate (₹)</span><input type="number" min="0" step="0.01" value={form.overtimeHourlyRate} onChange={(event) => update("overtimeHourlyRate", Number(event.target.value))} /></label><label><span>Effective from</span><input type="date" value={form.effectiveFrom} onChange={(event) => update("effectiveFrom", event.target.value)} required /></label><div className="rule-toggles"><label><input type="checkbox" checked={Boolean(form.paidLeave)} onChange={(event) => update("paidLeave", event.target.checked ? 1 : 0)} />Approved leave is payable</label><label><input type="checkbox" checked={Boolean(form.paidWeekOff)} onChange={(event) => update("paidWeekOff", event.target.checked ? 1 : 0)} />Week offs / holidays are payable</label></div></fieldset></form>
      <div className="settings-grid"><section className="panel"><div className="panel-heading"><div><span className="eyebrow">Excel salary register</span><h2>Earning components</h2></div><span className="muted-label">Edit inside each salary record</span></div><div className="tag-cloud">{earnings.map((item) => <span key={item}>{item}<i>Enabled</i></span>)}</div><div className="formula-box"><span>Gross earnings</span><code>Basic → Medical Allowance (OT hours excluded; OT wages included)</code></div></section><section className="panel"><div className="panel-heading"><div><span className="eyebrow">Excel salary register</span><h2>Deduction components</h2></div><span className="muted-label">Edit inside each salary record</span></div><div className="tag-cloud">{deductions.map((item) => <span key={item}>{item}<i>Enabled</i></span>)}</div><div className="formula-box"><span>Net payable</span><code>Gross − statutory − other − accommodation + return amount</code></div></section></div>
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Attendance controls</span><h2>Status code mapping</h2></div><span className="muted-label">Matches the original attendance workbook</span></div><div className="status-code-grid">{Object.entries(statusMeta).map(([code, meta]) => <article key={code}><b className={meta.className}>{code}</b><div><strong>{meta.label}</strong><span>{code === "P" ? "Counts as present and payable" : code === "A" ? "Counts as absent and follow-up" : code === "L" ? "Approved leave; paid according to saved rules" : code === "WO" ? "Any rostered weekday off" : code === "H" ? "Company holiday" : "Worked on WO/holiday; counts as present"}</span></div></article>)}</div></section>
      <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Approval governance</span><h2>Recommended maker-checker workflow</h2></div></div><div className="approval-grid"><article><span>1</span><div><strong>HR Operations</strong><p>Attendance, shift, week off, OT and employee master.</p></div></article><article><span>2</span><div><strong>Payroll Maker</strong><p>Earnings, deductions, accommodation and exception resolution.</p></div></article><article><span>3</span><div><strong>Accounts Checker</strong><p>Totals, statutory mapping, bank readiness and variance.</p></div></article><article><span>4</span><div><strong>Authorised Approver</strong><p>Final lock, payment export and audit trail.</p></div></article></div></section>
    </div>
  );
}

function PayrollDetail({ item, onClose, onPayslip, onEdit, locked }: { item: PayrollItem; onClose: () => void; onPayslip: () => void; onEdit: () => void; locked: boolean }) {
  const earnings = [["Basic", item.basic], ["DA", item.da], ["HRA", item.hra], ["Conveyance", item.conveyance], ["Food allowance", item.foodAllowance], ["Night allowance", item.nightAllowance], ["OT wages", item.overtimeWages], ["Attendance bonus", item.attendanceBonus], ["Arrears", item.arrears], ["Holiday wages", item.holidayWages], ["Production incentive", item.productionIncentive], ["Medical allowance", item.medicalAllowance]] as Array<[string, number]>;
...
function PayslipSheet({ item, vendor, unit, run, period }: { item: PayrollItem; vendor: Vendor; unit: ClientUnit; run: PayrollRun | null; period: string }) {
  const earnings = configuredFields(unit.payslipEarningsJson, earningFields).map((field) => [readableField(field), Number(item[field as keyof PayrollItem] ?? 0)] as [string, number]);
  const deductions = configuredFields(unit.payslipDeductionsJson, deductionFields).map((field) => [readableField(field), Number(item[field as keyof PayrollItem] ?? 0)] as [string, number]);
  ...
  return <article className="payslip-sheet payslip-half-a4"><header>{vendor.logoDataUrl ? <img className="payslip-company-logo" src={vendor.logoDataUrl} alt={`${vendor.name} logo`} /> : <div className="payslip-logo">JOY</div>}<div><h2>EMPLOYEE SALARY SLIP</h2><strong>{monthLabel(period).toUpperCase()}</strong></div></header><section className="payslip-company"><h3>{employerTitle}</h3><p>{unit.payslipSubtitle ?? `${unit.unitName} · Payroll partner: ${vendor.legalName}`}</p><p>{unit.payslipAddress ?? unit.location}</p><span>{unit.payslipContact ?? "joyindia.in · info@joycorporatesolutions.com"}</span></section><div className="payslip-meta"><div><span>Employee ID</span><strong>{item.employeeCode}</strong></div><div><span>Employee name</span><strong>{item.employeeName}</strong></div><div><span>Department</span><strong>{item.department}</strong></div><div><span>Employer / unit</span><strong>{unit.clientName} · {unit.unitName}</strong></div><div><span>Payroll period</span><strong>{range ? `${range.start} to ${range.inclusiveEnd}` : monthLabel(period)}</strong></div><div><span>Working days</span><strong>{run?.workingDays ?? 26}</strong></div><div><span>Payable days</span><strong>{item.payableDays}</strong></div><div><span>OT hours</span><strong>{item.overtimeHours}</strong></div></div><div className="payslip-columns"><section><h4><span>Earnings</span><b>Amount</b></h4>{earnings.map(([label, value]) => <div key={label}><span>{label}</span><b>{value ? money(value) : "—"}</b></div>)}<footer><span>Gross earnings</span><b>{money(item.grossEarnings)}</b></footer></section><section><h4><span>Deductions</span><b>Amount</b></h4>{deductions.map(([label, value]) => <div key={label}><span>{label}</span><b>{value ? money(value) : "—"}</b></div>)}<footer><span>Total deductions</span><b>{money(item.totalDeductions)}</b></footer></section></div><div className="payslip-net"><div><span>Net payable</span><strong>{money(item.netPayable)}</strong></div><p><span>Amount in words</span>{numberToWordsIndian(Math.round(item.netPayable))} Rupees Only</p></div><footer className="payslip-footnote">*** {unit.payslipFooter ?? "This is a computer-generated payslip and does not require a physical signature."} ***</footer></article>;
}
...
function PayslipModal({ item, vendor, unit, run, period, canExport, onClose }: { item: PayrollItem; vendor: Vendor; unit: ClientUnit; run: PayrollRun | null; period: string; canExport: boolean; onClose: () => void }) {
  return <div className="modal-layer"><button className="modal-scrim" aria-label="Close payslip" onClick={onClose} /><div className="payslip-modal"><div className="modal-toolbar"><div><strong>Half-A4 salary slip</strong><span>{unit.clientName} · two slips can fit on one A4 sheet</span></div><div>{canExport ? <button className="secondary-button" onClick={() => window.print()}><Icon name="download" size={16} />Print / Save PDF</button> : null}<button className="icon-button" onClick={onClose}>×</button></div></div><PayslipSheet item={item} vendor={vendor} unit={unit} run={run} period={period} /></div></div>;
}

function BulkPayslipModal({ items, vendor, unit, run, onClose }: { items: PayrollItem[]; vendor: Vendor; unit: ClientUnit; run: PayrollRun; onClose: () => void }) {
  return <div className="modal-layer"><button className="modal-scrim" onClick={onClose} /><div className="payslip-modal bulk-payslip-modal"><div className="modal-toolbar"><div><strong>Bulk employee salary slips</strong><span>{items.length} employees · two half-A4 slips per A4 page</span></div><div><button className="primary-button" onClick={() => window.print()}>Print / Save bulk PDF</button><button className="icon-button" onClick={onClose}>×</button></div></div><div className="bulk-payslip-pages">{items.map((item) => <PayslipSheet key={item.id} item={item} vendor={vendor} unit={unit} run={run} period={run.payPeriod} />)}</div></div></div>;
}

function ProfileModal({ profile, busy, onClose, onSave, onChangePassword }: { profile: AppUserProfile; busy: boolean; onClose: () => void; onSave: (details: Record<string, unknown>) => Promise<void>; onChangePassword?: (password: string) => Promise<void> }) {
  const [password, setPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [passwordMessage, setPasswordMessage] = useState("");
  async function saveProfile(event: FormEvent<HTMLFormElement>) { event.preventDefault(); await onSave(Object.fromEntries(new FormData(event.currentTarget))); }
  async function changePassword() {
    setPasswordMessage("");
    if (password.length < 12) { setPasswordMessage("Password must contain at least 12 characters."); return; }
    if (password !== confirmPassword) { setPasswordMessage("The passwords do not match."); return; }
    if (!onChangePassword) { setPasswordMessage("Password change is available on the Supabase production login."); return; }
    try { await onChangePassword(password); setPassword(""); setConfirmPassword(""); setPasswordMessage("Password changed successfully."); } catch (error) { setPasswordMessage(error instanceof Error ? error.message : "Password could not be changed."); }
  }
  return <div className="modal-layer action-modal-layer"><button className="modal-scrim" aria-label="Close profile" onClick={onClose} /><div className="action-modal"><header className="action-modal-header"><div><span className="eyebrow">My account</span><h2>Profile & password</h2></div><button className="icon-button" onClick={onClose}>×</button></header><div className="action-modal-body"><form className="form-grid" onSubmit={(event) => void saveProfile(event)}><label><span>Employee code</span><input value={profile.employeeCode ?? ""} disabled /></label><label><span>Sign-in email</span><input value={profile.email} disabled /></label><label><span>Full name *</span><input name="fullName" defaultValue={profile.fullName ?? ""} required /></label><label><span>Department</span><input name="department" defaultValue={profile.department ?? ""} /></label><label><span>Mobile number</span><input name="mobileNumber" defaultValue={profile.mobileNumber ?? ""} /></label><button className="primary-button form-span" disabled={busy}>Save profile</button></form><section className="profile-password-panel"><h3>Change password</h3><label><span>New password</span><input type="password" minLength={12} value={password} onChange={(event) => setPassword(event.target.value)} /></label><label><span>Confirm password</span><input type="password" minLength={12} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} /></label>{passwordMessage ? <p>{passwordMessage}</p> : null}<button className="secondary-button" onClick={() => void changePassword()}>Change password</button></section></div></div></div>;
}
...
function EmptyPayroll({ unit, canCreate, canImport, canOpenClients, onGoToVendors, onCreate, onImport }: { unit: ClientUnit | null; canCreate: boolean; canImport: boolean; canOpenClients: boolean; onGoToVendors: () => void; onCreate: () => void; onImport: () => void }) {
  return <section className="panel empty-payroll"><span><Icon name="calculator" size={28} /></span><h2>{unit ? "No payroll run for this unit" : "No employer unit is available"}</h2><p>{unit ? `${unit.clientName} · ${unit.unitName} is ready. A permitted team member can create a monthly salary run or import an Excel workbook.` : "A permitted team member can add the client and its employer or factory unit."}</p><div>{unit && canCreate ? <button className="primary-button" onClick={onCreate}>Create payroll run</button> : null}{unit && canImport ? <button className="secondary-button" onClick={onImport}>Import Excel</button> : null}{canOpenClients ? <button className="secondary-button" onClick={onGoToVendors}>Open clients & employers</button> : null}{!canCreate && !canImport && !canOpenClients ? <span className="access-mode-note"><Icon name="eye" size={15} />View-only access</span> : null}</div></section>;
}

function readableField(value: string) {
  const overrides: Record<string, string> = { da: "DA", hra: "HRA", pfDeduction: "PF", esiDeduction: "ESI", lwf: "LWF", tds: "TDS", idCard: "ID card", aadhaarUpdate: "Aadhaar update", tshirt: "T-shirt", ifscMasked: "IFSC", uanMasked: "UAN", esiMasked: "ESI number" };
  return overrides[value] ?? value.replace(/([A-Z])/g, " $1").replace(/^./, (character) => character.toUpperCase());
}
...
function unitAttendanceCycle(period: string, unit: ClientUnit | null) {
  ...
  if (unit.attendanceCycleStartDay <= unit.attendanceCycleEndDay) return { start: date(year, month, unit.attendanceCycleStartDay), end: date(year, month, unit.attendanceCycleEndDay) };
  const previous = new Date(Date.UTC(year, month - 2, 1));
  return { start: date(previous.getUTCFullYear(), previous.getUTCMonth() + 1, unit.attendanceCycleStartDay), end: date(year, month, unit.attendanceCycleEndDay) };
}

function configuredFields(json: string | null | undefined, defaults: readonly string[]) {
...
function PayrollActionModal({ modal, vendors, units, hostels, accommodationTypes, rooms, employees, shifts, remarks, unit, vendorId, currentPeriod, currentRun, busy, onClose, onAction }: {
  ...
  busy: boolean;
  onClose: () => void;
  onAction: (action: string, message: string, details?: Record<string, unknown>) => Promise<boolean>;
}) {
  ...
  const [runWorkingDays, setRunWorkingDays] = useState(currentRun?.workingDays ?? unit?.attendanceWorkingDays ?? 26);
  const [runProcessingMode, setRunProcessingMode] = useState<"attendance" | "salary_import">("attendance");
  const [unitEarnings, setUnitEarnings] = useState<string[]>(configuredFields(employerUnit?.payslipEarningsJson, earningFields));
  const [unitDeductions, setUnitDeductions] = useState<string[]>(configuredFields(employerUnit?.payslipDeductionsJson, deductionFields));
  const [companyLogo, setCompanyLogo] = useState(client?.logoDataUrl ?? "");
  const [employeePhoto, setEmployeePhoto] = useState(employee?.photoDataUrl ?? "");
  ...
  const title = modal.kind === "vendor" ? client ? "Edit group company" : "Add group company" : modal.kind === "unit" ? employerUnit ? "Edit client unit" : "Add client unit" : modal.kind === "employee" ? employee ? "Edit employee" : "Add employee" : modal.kind === "employee-left" ? "Put employee left date" : modal.kind === "accommodation-type" ? selectedType ? "Edit accommodation type" : "Add accommodation type" : modal.kind === "shift" ? selectedShift ? "Edit shift details" : "Add shift" : modal.kind === "remark" ? selectedRemark ? "Edit remark" : "Add remark" : modal.kind === "app-user" ? accessProfile ? "Edit user access" : "Add user access" : modal.kind === "run" ? "Create payroll month" : modal.kind === "attendance" ? "Update attendance" : modal.kind === "salary" ? "Edit salary calculation" : modal.kind === "accommodation" ? "Edit accommodation recovery" : "Bulk import payroll data";
  const submitLabel = modal.kind === "import" ? "Import into payroll" : modal.kind === "run" ? "Create payroll run" : modal.kind === "vendor" ? client ? "Save group company" : "Add group company" : modal.kind === "unit" ? employerUnit ? "Save client unit" : "Add client unit" : modal.kind === "accommodation-type" ? selectedType ? "Save accommodation type" : "Add accommodation type" : modal.kind === "shift" ? selectedShift ? "Save shift" : "Add shift" : modal.kind === "remark" ? selectedRemark ? "Save remark" : "Add remark" : modal.kind === "app-user" ? accessProfile ? "Save user access" : "Add user" : "Save changes";
  ...
  async function inspectFile(file: File, period: string) {
    ...
    try {
      ...
      if (!result.employees.length) throw new Error("The workbook does not contain employee rows.");
      setParsed(result);
    } catch (error) {
      setParseError(error instanceof Error ? error.message : "Unable to read the selected workbook");
    } finally {
      setParsing(false);
    }
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (modal.kind === "vendor") await onAction("save-client", client ? "Group company details updated" : "Group company created", { ...fields, logoDataUrl: companyLogo, id: client?.id });
      ...
      await onAction("save-employee", employee ? "Employee details updated" : "Employee added to payroll", { employee: { ...fields, photoDataUrl: employeePhoto, shiftPattern: employeeShiftPattern, defaultShift: applicable[0], applicableShiftsJson: JSON.stringify(applicable), paymentMode: "bank", employmentType, id: employee?.id } });
    }
    ...
    else if (modal.kind === "app-user") await onAction("save-app-user", accessProfile ? "Direct employee user updated" : "Direct employee login created", { id: accessProfile?.id, email: fields.email, temporaryPassword: fields.temporaryPassword, fullName: fields.fullName, employeeCode: fields.employeeCode, department: fields.department, dateOfJoining: fields.dateOfJoining, mobileNumber: fields.mobileNumber, role: userRole, permissions: permissionDraft, canApprovePayroll: approvalDraft, clientScope: clientScopeDraft, unitScope: unitScopeDraft, hostelScope: hostelScopeDraft });
    else if (modal.kind === "run") await onAction("create-run", "Custom monthly payroll run created", { payPeriod: fields.payPeriod, periodStart: fields.periodStart, periodEnd: fields.periodEnd, workingDays: fields.workingDays, processingMode: runProcessingMode });
  ...
  return <div className="modal-layer action-modal-layer"><button className="modal-scrim" aria-label="Close form" onClick={onClose} /><form className={`action-modal ${modal.kind === "salary" || modal.kind === "app-user" ? "action-modal-wide" : ""}`} onSubmit={submit}><header className="action-modal-header"><div><span className="eyebrow">{modal.kind === "app-user" ? "Identity & module permissions" : unit ? `${unit.clientName} · ${unit.unitName}` : "Payroll configuration"}</span><h2>{title}</h2></div><button className="icon-button" type="button" aria-label="Close" onClick={onClose}><Icon name="close" /></button></header><div className="action-modal-body"><datalist id="payroll-remark-options">{remarks.filter((remark) => remark.status === "active").map((remark) => <option key={remark.id} value={remark.notes ? `${remark.title}: ${remark.notes}` : remark.title} />)}</datalist>
    ...
    {modal.kind === "employee" ? <div className="form-grid"><label><span>Employee code *</span><input name="employeeCode" defaultValue={employee?.employeeCode} placeholder="J1007" required /></label><label><span>Employee name *</span><input name="name" defaultValue={employee?.name} placeholder="Full name" required /></label><label><span>Department *</span><input name="department" defaultValue={employee?.department ?? "Production"} required /></label><label><span>Date of joining *</span><input name="dateOfJoining" type="date" defaultValue={employee?.dateOfJoining ?? `${currentPeriod}-01`} required /></label><label><span>Basic salary / daily rate (₹)</span><input name="salaryAmount" type="number" min="0" step="0.01" defaultValue={employee?.salaryAmount ?? 0} required /></label><label><span>Salary basis</span><select name="salaryBasis" defaultValue={employee?.salaryBasis ?? "monthly"}><option value="monthly">Monthly</option><option value="daily">Daily</option></select></label><label><span>Default shift</span><select name="defaultShift" defaultValue={employee?.defaultShift ?? shiftOptions.find((shift) => shift === "General") ?? shiftOptions[0]}>{shiftOptions.map((shift) => <option key={shift}>{shift}</option>)}</select></label><label><span>Payment mode</span><select name="paymentMode" defaultValue={employee?.paymentMode ?? "cash"}><option value="cash">Cash</option><option value="bank">Bank transfer</option></select></label><label><span>UAN</span><input name="uanMasked" defaultValue={employee?.uanMasked ?? ""} placeholder="Optional unless PF applies" /></label><label><span>ESI number</span><input name="esiMasked" defaultValue={employee?.esiMasked ?? ""} placeholder="Optional unless ESI applies" /></label><label><span>Bank account number</span><input name="bankAccountMasked" defaultValue={employee?.bankAccountMasked ?? ""} /></label><label><span>IFSC code</span><input name="ifscMasked" defaultValue={employee?.ifscMasked ?? ""} /></label><label><span>Bank name</span><input name="bankName" defaultValue={employee?.bankName ?? ""} /></label><label><span>EPF applicable</span><select name="pfApplicable" defaultValue={employee?.pfApplicable === 0 ? "no" : "yes"}><option value="yes">Required</option><option value="no">Not required</option></select></label><label><span>Customized EPF wage (₹)</span><input name="pfWageAmount" type="number" min="0" max="15000" step="0.01" defaultValue={employee?.pfWageAmount ?? 0} /><small>Maximum statutory ceiling ₹15,000</small></label><label><span>ESI applicable</span><select name="esiApplicable" defaultValue={employee?.esiApplicable === 0 ? "no" : "yes"}><option value="yes">Required</option><option value="no">Not required</option></select></label><label><span>Customized ESI wage (₹)</span><input name="esiWageAmount" type="number" min="0" max="21000" step="0.01" defaultValue={employee?.esiWageAmount ?? 0} /><small>Maximum coverage ceiling ₹21,000</small></label><label><span>Professional Tax</span><select name="ptApplicable" defaultValue={employee?.ptApplicable === 0 ? "no" : "yes"}><option value="yes">Applicable</option><option value="no">Not required</option></select></label><label><span>Tamil Nadu LWF</span><select name="lwfApplicable" defaultValue={employee?.lwfApplicable === 0 ? "no" : "yes"}><option value="yes">Applicable</option><option value="no">Not required</option></select></label><label><span>Accommodation type *</span><select name="accommodationType" value={employeeType} onChange={(event) => { setEmployeeType(event.target.value); setEmployeeRoomId(""); setEmployeeRoomNumber(""); }} required>{activeAccommodationTypes.map((type) => <option key={type.id} value={type.name}>{type.name}</option>)}</select></label><label><span>Existing room</span><select name="roomId" value={employeeRoomId} onChange={(event) => { const id = event.target.value; setEmployeeRoomId(id); setEmployeeRoomNumber(matchingRooms.find((room) => room.id === id)?.roomNumber ?? ""); }}><option value="">No existing room selected</option>{matchingRooms.map((room) => <option key={room.id} value={room.id}>{room.roomNumber}{room.capacity ? ` · capacity ${room.capacity}` : ""}</option>)}</select></label><label><span>Room number</span><input name="roomNumber" value={employeeRoomNumber} onChange={(event) => { setEmployeeRoomNumber(event.target.value); setEmployeeRoomId(""); }} placeholder="Choose above or type a new room number" /></label><label className="form-span"><span>Employee photo for ID card</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (!file) return; if (file.size > 500000) { window.alert("Choose an employee photo smaller than 500 KB."); return; } const reader = new FileReader(); reader.onload = () => setEmployeePhoto(String(reader.result ?? "")); reader.readAsDataURL(file); }} />{employeePhoto ? <small>Employee photo selected</small> : <small>Recommended: clear passport-style photo</small>}</label><label><span>Other remarks</span><input name="remarks" list="payroll-remark-options" defaultValue={employee?.remarks ?? ""} placeholder="Optional employee notes" /></label><div className="form-note form-span"><strong>QR employee ID card</strong><span>The ID card and employee QR code are generated automatically immediately after saving. Open Employees → Actions → ID card + QR to print it.</span></div></div> : null}
    ...
    {modal.kind === "app-user" ? <div className="user-access-form">
      {!accessProfile ? <label><span>Temporary password * (minimum 12 characters)</span><input name="temporaryPassword" type="password" minLength={12} required autoComplete="new-password" /></label> : null}
    ...
    {modal.kind === "run" ? <div className="form-grid"><label className="form-span"><span>Payroll processing type *</span><select value={runProcessingMode} onChange={(event) => setRunProcessingMode(event.target.value as "attendance" | "salary_import")}><option value="attendance">Attendance available in application</option><option value="salary_import">Import salary-slip / salary-register headers</option></select></label><label className="form-span"><span>Payroll month / label *</span><input name="payPeriod" type="month" value={runMonth} onChange={(event) => { const value = event.target.value; const dates = unitAttendanceCycle(value, unit); setRunMonth(value); setRunStart(dates.start); setRunEnd(dates.end); setRunWorkingDays(unit?.attendanceWorkingDays ?? 26); }} required /></label><label><span>Calculation period starts *</span><input name="periodStart" type="date" value={runStart} onChange={(event) => setRunStart(event.target.value)} required /></label><label><span>Calculation period ends *</span><input name="periodEnd" type="date" value={runEnd} onChange={(event) => setRunEnd(event.target.value)} required /></label><label className="form-span"><span>Monthly working days *</span><input name="workingDays" type="number" min="1" max="62" value={runWorkingDays} onChange={(event) => setRunWorkingDays(Number(event.target.value))} required /></label><div className="form-note form-span">Attendance mode calculates salary from saved attendance. Salary-import mode keeps imported earning and deduction headers as the payroll source.</div></div> : null}
    ...
    {modal.kind === "accommodation" ? <div className="form-grid"><label className="form-span"><span>Employee *</span><select name="employeeId" defaultValue={modal.employee?.id ?? employees[0]?.id} required>{employees.map((row) => <option value={row.id} key={row.id}>{row.employeeCode} · {row.name} · {row.accommodationType}</option>)}</select></label><label className="form-span"><span>Room number</span><input name="roomNumber" defaultValue={modal.charge?.roomNumber ?? modal.employee?.roomNumber ?? ""} /></label>{recoveryFields.map((field) => <label key={field}><span>{readableField(field)} (₹)</span><input name={field} type="number" min="0" step="0.01" defaultValue={modal.charge?.[field] ?? 0} /></label>)}</div> : null}

    {modal.kind === "import" ? <div className="import-form"><label><span>Import into payroll month *</span><input type="month" value={importPeriod} onChange={(event) => { const period = event.target.value; setImportPeriod(period); if (selectedFile && period) void inspectFile(selectedFile, period); }} required /></label><label className="file-drop"><input type="file" accept=".xlsx,.csv,.txt" onChange={(event) => { const file = event.target.files?.[0]; if (file) void inspectFile(file, importPeriod); }} /><Icon name="file" size={25} /><strong>{selectedFile?.name ?? "Choose Attendance, Salary Register, CSV, or TXT"}</strong><span>Supports Excel .xlsx, CSV and tab/pipe/comma-delimited TXT files with attendance or salary-slip headers.</span></label>{parsing ? <div className="form-note">Reading file and mapping employee records…</div> : null}{parseError ? <div className="form-error">{parseError}</div> : null}{parsed ? <div className="import-preview"><span className="eyebrow">File recognized · {parsed.sourceType === "salary" ? "Salary import mode" : "Attendance / employee mode"}</span><strong>{parsed.sheetName}</strong><div><span>{parsed.employees.length} employees</span><span>{parsed.attendance.length} attendance marks</span><span>{parsed.salaryItems.length} salary records</span></div><p>Records will be saved to {unit?.clientName} · {unit?.unitName} for {monthLabel(importPeriod)}. A payroll run will be created if one does not already exist.</p></div> : null}</div> : null}
  </div><footer className="action-modal-footer">{modal.kind === "attendance" && modal.entry ? <button className="danger-button" type="button" disabled={busy} onClick={() => { if (window.confirm(`Delete attendance for ${modal.employee.name} on ${modal.date}?`)) void onAction("delete-attendance", "Attendance and shift entry deleted", { employeeId: modal.employee.id, attendanceDate: modal.date }); }}>Delete entry</button> : null}{modal.kind === "accommodation" && modal.employee && modal.charge ? <button className="danger-button" type="button" disabled={busy} onClick={() => { if (window.confirm(`Delete accommodation recoveries for ${modal.employee?.name}?`)) void onAction("delete-accommodation", "Accommodation recoveries deleted", { employeeId: modal.employee?.id }); }}>Delete recovery</button> : null}<button className="secondary-button" type="button" onClick={onClose} disabled={busy}>Cancel</button><button className="primary-button" type="submit" disabled={busy || parsing || (modal.kind === "import" && !parsed)}>{busy ? <><span className="button-spinner" />Saving…</> : submitLabel}</button></footer></form></div>;
}

function EmptyState({ icon, title, detail }: { icon: string; title: string; detail: string }) {
  return <div className="empty-state"><span><Icon name={icon} /></span><div><strong>{title}</strong><p>{detail}</p></div></div>;
}

function EmployeeCell({ name, code, detail }: { name: string; code: string; detail: string }) {
  return <div className="employee-cell"><span>{initials(name)}</span><div><strong>{name}</strong><small>{code}{detail ? ` · ${detail}` : ""}</small></div></div>;
}

function FieldState({ value }: { value: string | null }) {
  return value ? <span className="field-ready"><Icon name="check" size={14} />{value}</span> : <span className="field-pending"><Icon name="alert" size={14} />Pending</span>;
}

function RecordActions({ status, onEdit, onToggle, onDelete, onOpen }: { status: string; onEdit: () => void; onToggle: () => void; onDelete: () => void; onOpen?: () => void }) {
  return <div className="record-actions">{onOpen ? <button className="record-action" onClick={onOpen}>Open</button> : null}<button className="record-action" onClick={onEdit}>Edit</button><button className="record-action" onClick={onToggle}>{status === "active" ? "Deactivate" : "Activate"}</button><button className="record-action record-delete" onClick={onDelete}>Delete</button></div>;
}

function StatusPill({ status }: { status: string }) {
  const key = status.toLowerCase().replace(/\s+/g, "-");
  return <span className={`status-pill pill-${key}`}><i />{status}</span>;
}
...
function downloadXlsx(filename: string, rows: Array<Array<string | number>>) {
  ...
  const column = (index: number) => { let label = ""; for (let value = index + 1; value; value = Math.floor((value - 1) / 26)) label = String.fromCharCode(65 + ((value - 1) % 26)) + label; return label; };
  const sheetRows = rows.map((row, rowIndex) => `<row r="${rowIndex + 1}">${row.map((cell, cellIndex) => { const ref = `${column(cellIndex)}${rowIndex + 1}`; return typeof cell === "number" ? `<c r="${ref}"${rowIndex === 0 ? ' s="1"' : ""}><v>${cell}</v></c>` : `<c r="${ref}" t="inlineStr"${rowIndex === 0 ? ' s="1"' : ""}><is><t>${xml(String(cell))}</t></is></c>`; }).join("")}</row>`).join("");
  const files: Record<string, Uint8Array> = {
    ...
    "xl/workbook.xml": strToU8('<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Indian Bank Bulk Upload" sheetId="1" r:id="rId1"/></sheets></workbook>'),
    ...
    "xl/worksheets/sheet1.xml": strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols><col min="1" max="1" width="12" customWidth="1"/><col min="2" max="2" width="24" customWidth="1"/><col min="3" max="4" width="20" customWidth="1"/><col min="5" max="5" width="14" customWidth="1"/><col min="6" max="9" width="24" customWidth="1"/></cols><sheetData>${sheetRows}</sheetData></worksheet>`),
  };
  const workbookBytes = new Uint8Array(zipSync(files));
  const url = URL.createObjectURL(new Blob([workbookBytes.buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }));
  const anchor = document.createElement("a"); anchor.href = url; anchor.download = filename; anchor.click(); URL.revokeObjectURL(url);
}
...
function numberToWordsIndian(value: number): string {
  ...
  const underHundred = (number: number) => number < 20 ? ones[number] : `${tens[Math.floor(number / 10)]}${number % 10 ? ` ${ones[number % 10]}` : ""}`;
  const underThousand = (number: number) => number < 100 ? underHundred(number) : `${ones[Math.floor(number / 100)]} Hundred${number % 100 ? ` ${underHundred(number % 100)}` : ""}`;
  const groups: Array<[number, string]> = [[10000000, "Crore"], [100000, "Lakh"], [1000, "Thousand"]];
  let remaining = value;
  const words: string[] = [];
  for (const [size, label] of groups) {
    const count = Math.floor(remaining / size);
    if (count) { words.push(`${underThousand(count)} ${label}`); remaining %= size; }
  }
