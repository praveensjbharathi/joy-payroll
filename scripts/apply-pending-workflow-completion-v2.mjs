...
async function patch(path, transform){const full=join(root,path);const source=await readFile(full,"utf8");const updated=transform(source);if(updated!==source)await writeFile(full,updated,"utf8");}
function replaceIf(source, search, replacement){return source.includes(search)&&!source.includes(replacement)?source.replace(search,replacement):source;}

await patch("app/employee-id-card.tsx", source => {
  source = source.replace(/\s*<button className="secondary-button" onClick=\{preparePrint\}>Print front \+ back<\/button>\s*/g,"\n            ");
  source = source.replaceAll("HR Download Front JPEG","Download Front HQ JPG").replaceAll("HR Download Back JPEG","Download Back HQ JPG");
  return source;
});

await patch("app/hostel-master.tsx", source => {
  if(!source.includes("hostel-edit-${selected.id}")){
    source=source.replace(/<form(\s+)className="form-grid"(\s+)onSubmit=\{updateHostel\}>/, '<form key={`hostel-edit-${selected.id}`} className="form-grid" onSubmit={updateHostel}>');
  }
  return source;
});

await patch("db/schema.ts", source => {
  if(!source.includes('approvalManagerEmail: text("approval_manager_email")')) source=source.replace('  canApprovePayroll: integer("can_approve_payroll").notNull().default(0),','  canApprovePayroll: integer("can_approve_payroll").notNull().default(0),\n  approvalManagerEmail: text("approval_manager_email"),\n  approvalSequence: integer("approval_sequence").notNull().default(0),');
  return source;
});

await patch("app/api/app-data/route.ts", source => {
  source=replaceIf(source,'    canApprovePayroll: role === "super_admin" || Boolean(row.canApprovePayroll),\n    createdAt: row.createdAt,','    canApprovePayroll: role === "super_admin" || Boolean(row.canApprovePayroll),\n    approvalManagerEmail: row.approvalManagerEmail,\n    approvalSequence: row.approvalSequence,\n    createdAt: row.createdAt,');
  source=replaceIf(source,'    canApprovePayroll: canApprovePayroll ? 1 : 0,\n    updatedAt: now,','    canApprovePayroll: canApprovePayroll ? 1 : 0,\n    approvalManagerEmail: role === "super_admin" ? null : optionalValue(payload.approvalManagerEmail)?.toLowerCase() ?? null,\n    approvalSequence: role === "super_admin" ? 999 : positiveValue(payload.approvalSequence ?? 0, "Approval sequence", 999),\n    updatedAt: now,');
  return source;
});

await patch("app/payroll-app.tsx", source => {
  source=replaceIf(source,'  canApprovePayroll: boolean;\n  createdAt: string;','  canApprovePayroll: boolean;\n  approvalManagerEmail: string | null;\n  approvalSequence: number;\n  createdAt: string;');
  source=replaceIf(source,'          canApprovePayroll: approvalDraft,\n          clientScope: clientScopeDraft,','          canApprovePayroll: approvalDraft,\n          approvalManagerEmail: fields.approvalManagerEmail,\n          approvalSequence: fields.approvalSequence,\n          clientScope: clientScopeDraft,');
...
await patch("app/reports-recovery.tsx", source => {
  if(!source.includes("<th>Employer unit</th>")){
    ...
    source=source.replace('{employeeRows.map(\n                ({ employee, charge, individual, shared, item }) => {','{employeeRows.map(\n                ({ employee, charge, individual, shared, item }, rowIndex) => {');
    ...
    const newCells=`                      <td>{rowIndex + 1}</td>\n                      <td><strong>{employee.name}</strong><small>{employee.employeeCode}</small></td>\n                      <td>{(() => { const u = data.units.find((entry) => entry.id === employee.clientUnitId); return u ? \`${'${u.clientName} · ${u.unitName}'}\` : "—"; })()}</td>\n                      <td>{employee.roomNumber ?? "—"}</td><td>₹{preRecovery.toFixed(2)}</td>\n                      <td>₹{(charge?.rent ?? 0).toFixed(2)}</td><td>₹{(charge?.bus ?? 0).toFixed(2)}</td><td>₹{(charge?.food ?? 0).toFixed(2)}</td><td>₹{(charge?.advance ?? 0).toFixed(2)}</td><td>₹{(charge?.idCard ?? 0).toFixed(2)}</td><td>₹{(charge?.medical ?? 0).toFixed(2)}</td><td>₹{(charge?.ticket ?? 0).toFixed(2)}</td><td>₹{(charge?.shoe ?? 0).toFixed(2)}</td><td>₹{(charge?.aadhaarUpdate ?? 0).toFixed(2)}</td><td>₹{(charge?.bankAccountCharge ?? 0).toFixed(2)}</td><td>₹{(charge?.tshirt ?? 0).toFixed(2)}</td><td>₹{(charge?.oldPending ?? 0).toFixed(2)}</td><td>₹{(charge?.gasShare ?? 0).toFixed(2)}</td><td>₹{(charge?.rationShare ?? 0).toFixed(2)}</td><td>₹{(charge?.provisionShare ?? 0).toFixed(2)}</td><td><strong>₹{(individual + shared).toFixed(2)}</strong></td><td>₹{(charge?.returnAmount ?? 0).toFixed(2)}</td><td><strong>₹{(item?.netPayable ?? 0).toFixed(2)}</strong></td>`;
    source=source.replace(oldCells,newCells);
  }
  source=source.replaceAll("Print / Save PDF","Print recovery slip · A4 portrait");
  return source;
});
