...
// Ensure the SMTP sender targets the dedicated salary-slip-mailer Edge Function.
source = source.replace(
  `      const response = await fetch(apiEndpoint, {\n`,
  `      const mailEndpoint = apiEndpoint.includes("/functions/v1/")\n        ? apiEndpoint.replace(/\\/functions\\/v1\\/[^/?]+/, "/functions/v1/salary-slip-mailer")\n        : "https://fsiinadrkhsfzuheckbp.supabase.co/functions/v1/salary-slip-mailer";\n      const response = await fetch(mailEndpoint, {\n`,
);
source = source.replace(
...
console.log("Salary-slip email button is always visible in the payslip toolbar and sends the PDF through the dedicated SMTP mailer.");
